 ...
 * It ain't perfect. It just is what it is.
 *
...
class RecentFiles implements Closeable {
	...
	// I blew this out to from 100 to 1000, coz it really does not matter, it
	// just makes the scrollbar a bit useless, thats all. And thats mainly
	// because large ScrollBars are bit ____in useless. Sigh.
	private static final int MAX_SIZE = 1000;

	// a List of SourceIDs to process
	private final ArrayList<SourceID> list = new ArrayList<>(MAX_SIZE);

	// the array of SourceIDs vended to the GUI
	...
	private RecentFiles() {
		if ( IO.RECENT_FILES.exists() ) {
			try ( BufferedReader reader = new BufferedReader(
				...
				StdErr.whinge("WARN: "+Log.me()+" IOException", ex);
			}
		}
	}
	 ...
	 * puzzleID so that you can chain with me. If the $pid is null or empty
	 * then returns EMPTY_PUZZLE_ID; not null, as you might expect.
	 */
	SourceID add(final SourceID pid) {
		if ( pid==null || pid.file==null )
			return EMPTY_PUZZLE_ID;
		final int i = list.indexOf(pid);
		if ( i == 0 )
			return pid;
