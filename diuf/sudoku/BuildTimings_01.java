...
/**
 * The BuildTimings_* exists specifically to hold this over-large comment.
 ...
 * logic, to see if were making progress in the quest to solve all possible
 * Sudoku puzzles as simply, accurately, and quickly as possible.
 * <p>
 * That and its just nice to remember stuff like it took Juillerat 17:48 to
 ...
 * <p>
 * This comment was moved from the LogicalSolver class comments because its
 ...
 * NB: The first 200-or-so puzzles in top1465.d5.mt are the hardest Ive ever
 *     seen. I suppose they mustve been handmade specifically to be HARD.
 * NB: Ive just run the original project by Nicolas Juillerat on this box, for
 ...
 *    24 seconds Id take it in a second.
 * </pre>
 * <hr><p>
 ...
 *    could get A4E down to 24 seconds Id take it in a second." Well I got it
 *    down to 0.338 seconds, but only because its running after the "normal"
 ...
 *    A4E, and A5E ran before UnaryChain because theyre now fast enough, thanks
 ...
 *    Still seeing the occassional irrelevant hint, so thats next on my todos.
 * </pre>
 * <hr><p>
 ...
 * so, but first 3#top1465.d5.mt which took 33,743,216,168 ns. I dont like it!
 * </pre>
 * <hr><p>
 ...
 * maxCandidates so we dont try again until a candidate is killed elsewhere.
 * </pre>
 * <hr><p>
 ...
 *    where "top1465.d5" is the input file name minus its extension (.mt)
 ...
 * but before and after timings tell me theres something wrong with the
 * profilers performance stats, like maybe the profiler itself is screwing
 * the timings of the tiny/fast methods its timing.<br>
 ...
 * and lumps all the others into the callers self-time, so it wasnt actually
 ...
 * which lead to its inception was flawed buy my misunderstanding of what
 ...
 * NOTES:
 * 1. Were under 10 minutes with A8E enabled, which is gratifying, but half
 *    that would be a LOT better. Were only solving Sudoku puzzles, not
 *    waiting for The London Bus. Maybe its a bit of both. Sigh.
 * </pre>
 * <hr><p>
 * <b>KRC 2019-10-15</b> Added A9E and A10E, and Im stopping there.
 * In -REDO mode A3..10E are now invoked only when theyre going to find the
 * next hint. Ive got it parsing the hintNumbers of each A*E out of a keeper
 * log. Its a hack, reading a BIG part of the solution out of a previous log,
 * but its DAMN fast!
 * <pre>
 *        time(ns)  calls   time/call   elims   time/elim hinter
 ...
 * NOTES:
 * 1. 2 minutes and 12 seconds with all A*Es is, umm, damn fast. FIGJAM!
 *    But of course its a total HACK, coz it relies on having solved these
 *    puzzles previously. But I enjoyed the challenge of writing it.
 * 2. Without -REDO its still 1465 in 2,558,947,881,721 (42:38) @ 1,746,722,103
 * </pre>
 * <hr><p>
 * <b>KRC 2019-10-17</b> I generalised the mechanism to de/activate hinters
 * into INumberedHinter and applied it to Chainer which now extends
 * AHintNumberActivatableHinter which extends AHinter. Its a bit faster again.
 * Now Im thinking of applying it to all the heavies, but I think were going
 ...
 * NOTES:
 * 1. 1 minute and 38 seconds is pretty damn fast, but of course its a total
 *    HACK, coz it relies on having solved these puzzles previously.
 * 2. Without -REDO its still 40-odd minutes, ie slower than a wet week.
 * </pre>
 * <hr><p>
 * <b>KRC 2019-10-26</b> Ive applied INumberedHinter to ALL the heavier
 * hinters, and its slower than applying it to just the uber-heavies,
 * but Im keeping it anyway, and Ill build and release now.
 * <pre>
 * There was a LOG here but it was rooted, because the keeper didnt match the
 * software version, so it fell-through to DynamicPlus (the catch all), so Ive
 ...
 * while were still building the combo, so that an excluded combo is checked
 * once, not multiple times: once for each subsequent value in the combo. Its
 * touch and go how often it should be checked. An enthusiast could dick around
 * with it to see whats actually fastest. I might even do that, but not today.
 * <pre>
 *       time(ns)  calls   time/call   elims   time/elim hinter
 ...
 *    running slower because its so hot this afternoon.
 *    2019-11-14 1465 in 132,878,014,439 (02:12) @ 90,701,716 -REDO
 *    Thats even slower than yesterday! I can feel a reversion coming on.
 * 2. Just noticed that Dynamic Plus took 20 seconds in the previous run, which
 *    means that the keeper log didnt "match" the software so it didnt run the
 *    right hinter at the right time, which makes this slower run even more of
 *    a worry. Ill run it again tonight, when its cooler.
 *    This just in: Yep, The previous run was rooted. Ive removed the log.
 * 3. Ive just thought, I changed AUTOSOLVE in apply hint, so that we see the
 *    naked singles again. Its definately slower, but it cant be that much
 ...
 *    but I still think there has to be a fundamentally faster way, that doesnt
 *    involve repeatedly examining values thatre already known to be allowed.
 ...
 *    it could take just one pass to allow-all its values; upon which we throw,
 ...
 *    way through as the focus we know weve found an exclusion (like now) but
 *    we dont spend ken-ages checking and rechecking new combinations of values
 *    thatre already known to be allowed. So yeah, the trick is my new beaut
 *    ute: AllAllowedException.
 * 2. REDO 1465 in 124,784,829,619 (02:04) @ 85,177,358 which aint too shabby.
 * </pre>
 * <hr><p>
 ...
 * its doing more of the work less often. Its 10 minutes faster overall.
 * <pre>
 * NOTE: This isnt a -REDO run
 *          time(ns)  calls  time/call  elims      time/elim hinter
 ...
 * but only when theyre likely to be empty.
 * <pre>
 *          time(ns)  calls  time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. Its only a minute faster than previous. Were getting close to the point
 *    of diminishing returns again. Ill need to do something fundamentally
 ...
 * each common excluder bits which contains a value that isnt in the maybes of
 * any of the cells in the aligned set, because it can not be covered, and
 * therefore this common excluder can not contribute to an exclusion... its
 ...
 * NOTES:
 * 1. Its 3 minutes faster than previous, which is pretty good considering
 *    were on the point of diminishing returns.
 ...
 *
 * NOTES:
 * 1. Its about three and a half minutes faster than previous, which is good.
 * 2. But my previous comments regarding needing a complete rethink of A*E still
 *    stand. If its going to get much faster then Im going to need to dream up
 *    a way of doing it atleast an order of magnitude more efficiently, and
 *    frankly, Im too stupid. This is about as good as I know how to make it.
 * 3. The original Juillerat took 27 minutes 43 seconds to solve the same 1465
 *    puzzles on this box. So Im now faster doing A4..10E.
 * 3. REDO 1465 in 120,263,169,030 (02:00) @ 82,090,900
 *
 * Heres a summary of how it slows down as we enable each large A*E.
 ...
 * </pre>
 * <hr><p>
 * <b>KRC 2019-12-12</b> Foreskin me fungus, its been a roller-coaster ride
 ...
 * same changes as DiufSudoku_V6_30.035.2019-12-10.OK.7z plus Ive been off on
 ...
 * because theyre just too slow when correct.
 *
 * Ill release this as version 6_30.036 this afternoon, killing-off 035.
 *
 * Then its time to go to work on A7E_1C, to get its runtime down to acceptable
 * limit, so lets arbitrarily choose an hour for the whole runtime; so we must
 * promulgate any learnings to the other A*Es also.
 * <pre>
 *          time(ns)  calls   time/call  elims     time/elim hinter
 ...
 *
 * NOTES:
 * 1. 2 hours and 9 minutes is seriously slow, but its using correct A4E, A5E,
 *    and A6E, so its not as bad as it looks. I find that this configuration
 *    is still usable in the GUI, its just really slow for around 50 puzzles
 ...
 *    each cell when all of its potential values have been allowed, while
 *    retaining its covers functionalty somehow: a bitset-based approach.
 * 3. REDO isnt relevant to these changes, which are about accuracy over speed.
 * </pre>
 * <hr><p>
 * <b>KRC 2020-01-08</b> I just ran this for comparison with the above comment,
 * to assure myself that Im making some progress.
 * <pre>
 *         time(ns)  calls   time/call  elims      time/elim hinter
 ...
 * NOTES:
 * 1. Yes 1:39 is better than 2:09, but its only 20 minutes of the hour
 *    that I have to find. 1 hour is my arbitrary cap for top1465.d5.mt.
 *    I still feel that a complete rethink is required, and I still havent
 *    dreamt-up a faster solution to the basic A*E problem. Im still just
 *    "blindly" testing every possible combo; and I still keep thinking "there
 *    just HAS to be a better way", to skip "obviously unsucessful combos" a
 *    LOT faster; but every stat I can think of fails: it either takes longer
 *    to calculate than just blindly iterating the combos, or its non-bloody-
 *    deterministic, or both.
 *    So Im at an impass: I either need to have a genius moment right now,
 ...
 *    Maybe A6E and A7E will have to wait for CPUs to double in speed again;
 *    but unfortunately thats not ever likely to happen. Cores are as fast
 *    now as they will ever be, theyre just adding more cores and claiming
 ...
 *    really complicated really fast so Im afraid that Im too afraid to try.
 *    This is where Id google "Java thread queue" if I could afford a bloody
 ...
 *    A Time In The West"... Theyre (definately) still gonna get you if you
 *    dont do something. If you do something theyre probably still going to
 *    get you, but maybe not... Lets try to concentrate on the maybe. Hell,
 *    its only a bloody Sudoku puzzle solver, not an ICBM guidance system,
 *    which, IMHO, is probably simpler. Deadlier, but simpler.
 * 3. REDO isnt relevant.
 * </pre>
 * <hr><p>
 * <b>KRC 2020-01-08</b> Im still trying to make the "correct" version of
 * Aligned 7+ Exclusion fast enough to use in practice, and Im still failing
 ...
 * slow... Id happily accept an hour at this stage, and half an hour makes the
 * GUI "snappy". Tell him hes dreaming.
 * <pre>
 *          time(ns)  calls   time/call  elims      time/elim hinter
 ...
 *    Id like to cap that at 3 seconds, which is about where kids presume
 *    theyve "broken it", and start looking to there peers for reassurance.
 * 3. REDO isnt relevant.
 * </pre>
 * <hr><p>
 * <b>KRC 2020-01-17</b> Im still still trying to make the "correct" version of
 * Aligned 7+ Exclusion fast enough to use in practice, and Im still failing.
 ...
 *    that the three and a half-hours I was hoping for. Its not the required
 *    complete rethink, just refinements, using various stats to skip unhintable
 *    aligned sets.
 * 2. Im shipping it as version 6.30.040 2020-01-17 12:38:40.
 * 3. REDO isnt relevant.
 * </pre>
 * <hr><p>
 ...
 * FYI: Im comparing timings to those from <b>KRC 2019-11-29</b>.
 * <pre>
 *        time(ns)  calls  time/call  elims      time/elim hinter
 ...
 * NOTES:
 * 1. 2019-11-29 took 24:51, compared to now 13:21, but dont get too excited
 * 2. Im going to release this as version 6.30.040 2020-01-17 12:38:40.
 * 3. REDO isnt relevant.
 * </pre>
 * <hr><p>
 ...
 * NOTES:
 * 1. Its 30.0c and 95.8% humidity. The ceiling fan is on high, and my balls
 ...
 * make them final back to "externalised" automatic variables (ie how youd do
 ...
 * Ive exhumed the siblingsCount filter to the mainline to apply it earlier,
 * and promulgated that to A7E_2H, but Im not going to bother promulgating it
 * to the other A*Es (its too much of a pain in the ass).
 * <pre>
 *          time(ns)  calls   time/call  elims      time/elim hinter
 ...
 * 2. Im shipping it as version 6.30.043 2020-01-31 19:08:43
 * </pre>
 * <hr><p>
 ...
 *    Ill be satisfied if/when I get it down to an hour, so I still have to
 ...
 *    that Im missing the magic, but Im missing it, because Im not magic.
 * </pre>
 * <hr><p>
 ...
 * representative of performance for all top1465. It isnt, apparently. Sigh.
 * <pre>
 *          time(ns)  calls   time/call  elims      time/elim hinter
 ...
 * </pre>
 * <hr><p>
 * <b>KRC 2020-02-02</b> Just replaced A7E_1Cs avbs array with a virtual array.
 * Hard to believe such a simple thing cost 3 minutes. Theres a lesson. I shall
 ...
 *    has finished its examination. Piece of s__t! C# anyone? Except I dont
 ...
 * 4. Recall 2020-02-01 "no idea where to go from here?" Well lets: every combo
 ...
 *    weve found an exclusion, like now except were not spending ken-ages
 *    blindly testing and retesting each and every possible combination of
 *    values thatre ALL already bloody-well known to be allowed.
 *    Mathematically: Im trying to fail linearly 7+6+6+3+3+2 =    27
 ...
 * the any* technique to update the avb VIRTUAL array. Its time to release to
 * get a fresh 7z. Heres the latest timings.
 * <pre>
 *        time(ns)  calls  time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. The latest 11:52 (all hacked) is comparable to 2019-11-29 25:44, so its
 ...
 *    these puzzles because theyre the ones I could find for free, quickly.
 ...
 * Im angling for the 100 minute barrier.
 * <pre>
 *          time(ns)  calls   time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. We found 59 of 60 A8E hints, and 3 of 4 A10E hints, so its wrong, so I
 ...
 * Stop at max successful alignedSetCount in ALL A*Es.
 * <pre>
 *          time(ns)  calls   time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. All hint counts are as expected, and were under 100 minutes. Yeah!
 ...
 * So isAnyAllowed123456 reduces an O(7*7*7*7*7*7=117649) to just O(1ish) for
 * success but still O(117649) for failure, and heres a table of the number of
 ...
 * before the larger (isAnyAllowed123456/23456) methods are invoked, so theyre
 * NOT compiled (theyre interpreted, which is slower) but at least they call
 ...
 * 4. Next is A8E_2H then A8E_1C if Im not dead yet.
 * </pre>
 * <hr><p>
 ...
 * NOTES:
 * 1. Forget the time saved for now, because were missing hints. I suspect all
 ...
 *    Aligned8Exclusion_1C, thats the complete and correct version.
 * 2. Im not going to promulgate to A9E or A10E. Theres NO chance of getting
 *    the correct versions down to a "reasonable" runtime, so why bother?
 * 3. So I think Im done, finally. I can say quite honestly that I dont know
 ...
 * 4. So Im doing my final release DiufSudoku_V6_30.048.2020-02-11.OK.7z
 *    6.30.048 2020-02-11 20:39:48 isAnyAllowed* in A8E_2H and A8E_1C, and
 *    thats as far as Im going to push it, so thisll be my last release.
 *    Its been, umm, not fun exactly, more engaging.
 ...
 * </pre>
 * <hr><p>
 * <b>KRC 2020-02-13 13:18</b> Im playing with an "automatic" Filter class in
 ...
 * </pre>
 * <hr><p>
 * <b>KRC 2020-02-16 16:39</b> Heres what I guess is the final A8E_1C run.
 * I shall use the hacked version from now on. Ive given up on trying to make
 * the correct version fast enough to be practicably usable. Its useless.
 * <pre>
 *          time(ns)  calls   time/call  elims     time/elim hinter
 ...
 *    top1465.d5.mt with A2345678E_1C + A910E_2H in 112 minutes. Its not the
 ...
 * </pre>
 * <hr><p>
 * <b>KRC 2020-02-19 21:32</b> Heres a log from this mornings run, which Ive
 * been hacking the s__t-out of all day, and now its taking like 12 hours.
 * So heres proof that it happened, even if it isnt reproducable.
 * <pre>
 ...
 * in order to apply "all correct" in a reasonable time-frame. So Im done now.
 * The challenge was to solve these 1465 puzzles "as simply as possible" in
 * under and hour, and Ive sort-of done that. Im relying on knowledge gained
 * from a previous run, which is cheating. Id need a radically faster aligned
 * set exclusion algorithm to do better: a fast way to filter-out aligned-sets
 * which cant produce a hint, and Im not bright enough to dream it up; so I
 * gave up and cheated.
 * <pre>
 * LogicalSolver: unwanted: DirectNakedPair
 * LogicalSolver: unwanted: DirectHiddenPair
 ...
 * NOTES:
 * 1. Under an hour with all A*Es correct, but Im cheating!
 * 2. Ill do my final "correct" release now, and then its back to work on the
 ...
 *    Thats tricky coz each getHints call cant know if the next call will
 *    hint or not, so itll have to be done either in the LogicalSolver or the
 ...
 * A: The whole puzzle solves totally differently using all-hacked A*Es so
 *    we dont get to the puzzle that produces this hint. When you paste the
 *    puzzle into the GUI it is found (without filtering), so not broken.
 * NOTES:
 * 1. I have no idea if thats the fastest its ever been with all A*Es hacked
 *    but I reckon 4:26 is "acceptable", so Im shipping it.
 ...
 * AChainerBase offToOns. First I ran all hacked with IS_HACKY=false.
 * <pre>
 * LogicalSolver: unwanted: DirectNakedPair
 * LogicalSolver: unwanted: DirectHiddenPair
 * LogicalSolver: unwanted: DirectNakedTriple
 ...
 * <p>
 * So A9E_1C has taken 19.8 times longer than A8E_1C, and thats just wrong!
 * I still havent decided wether or not Im going to kill the run, but Im
 * leaning towards the affirmative. Im just going to take a look at the code
 * to see if Ive left a breakpoint or some other stupidity in there.
 * <pre>
 ...
 * 2. Thatll do pig. Coz itll bloody well have to.
 * </pre>
 * <hr>
 * <p>
 ...
 *    and finally I think its time DiufSudoku_V6 was released into the wild.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. Next Ill combine the 3 filters: hitSet, windex, and filter. so it only
 ...
 *    is FAR too slow for practical use, obviously, but its the best I know how
 ...
 * EVER calling a method in a tight loop would be substantially faster, but its
 * actually substantially slower, which I dont understand. Experience tells me
 ...
 * and thats just from w/o HitSet: 3 = 5,655 of 385,289 = 1.47% of use cases!
 * now now Ive tried making the paramters to covers method all final, in the
 ...
 *
 * So yes, I should reinstate the covers methods in all A*Es. Itll take me
 ...
 * NOTES:
 * 1. 1:47 is fast enough, even though Im cheating like a mo-fo.
 * 2. Release 6.30.058 2020-03-19 18:22:58
 *    => DiufSudoku_V6_30.058.2020-03-19.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2020-03-19 18:22:58</b> Pushed A5+Es "standard" counters up into
 ...
 * 3. Where to next I do not know exactly. Ill have a look at hobiwans Finned
 * Fish and see if I can pinch the concepts (but not the code) and tack them
 * onto Jullerats fisherman.
 * </pre>
 * <hr>
 * <p>
 ...
 * which wouldve been pretty useless anyway because Im too stupid to follow
 * it.
 * <pre>
 *        time(ns)  calls  time/call  elims      time/elim hinter
 ...
 * NOTES:
 * 1. 10:00 40 seconds slower, which aint great, but there you have it. My
 ...
 * 3. Im pretty sure there must be some bugs remaining in all of my HoDoKu
 *    boosting, so I guess I should put some test-cases in next. Im NOT going
 *    to attempt HoDoKus "advanced" Fishing techniques: (Finned) Franken Fish,
 *    (Finned) Mutant Fish, Siamese Fish, and Shark Finned Franken
 *    Swordfish can go____themselves (roughly) as far as Im concerned. Finned
 *    Fish was complex enough for me, and theyre defined as "simple" in
 ...
 * 3. Ill Probably look at Sashimi next, although I have no idea how to find
 *    them.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. I guess Ill look around HoDoKu to see what else I can boost. I might get
 ...
 *    coz its got a "dodgy" check in it to detect and throw-away invalid hints
 *    which break the puzzle, and one really should not need to do that, should
 *    one... so its still a piece of s__t, I just want it in the tin before I
 ...
 * 3. Problems described above are my "to do" list.
 * </pre>
 * <hr>
 * <p>
 ...
 *    ALS-XY-Wing is still rooted, and I hope thatll improve when its fixed.
 *    I just want a backup before I hack it, hence I release. That and its
 ...
 *    drop it from my wanted hinters list. Ive had my fun there.
 * 2. Release 6.30.064 2020-04-30 10:41:04
 *    => DiufSudoku_V6_30.064.2020-04-30.7z
 * 3. Next cab off the rank is why doesnt ALS-XY-Wing find any hints?
 * </pre>
 * <hr>
 * <p>
 ...
 * in DIUF. It works AND its faster than my hack implementation.
 * <pre>
 *         time(ns)  calls  time/call  elims      time/elim hinter
 ...
 *    than his previous, so its still "a bit s__t", but ALS-XY-Wing improved
 *    things, just not as much as Id hoped they would.
 * 2. Note that: Bi-Uni Grave, Aligned Pair and Aligned Oct no longer find any
 *    hints, so Ive unwanted these hinters. Ive had my fun with them.
 ...
 * NOTES:
 * 1. 12:09 is about 30 seconds faster than previous, so its still about 5
 ...
 * 3. Dont know what, if anything, I might do next. Im sick of ALSs, but I
 ...
 * NOTES:
 * 1. 11:57 is about 10 seconds faster than previous, so its still about 5
 *    minutes slower than before ALSs: ie "a bit s__t", just less s__t.
 ...
 *    HdkAlsXyWing and HdkAlsXyChain and work-out how/why theyre invalid and
 *    bloody-well fix the bloody bugs! Wish me luck. Ill need it. This s__t is
 ...
 * NOTES:
 * 1. 12:15 is about 15 seconds SLOWER than previous. Its all crap Ray.
 * 2. Release 6.30.068 2020-05-26 10:16:08
 *    => DiufSudoku_V6_30.068.2020-05-26.7z
 * 3. Next I create test-cases for HdkAlsXyChain and HdkAlsXyWing. Then I dont
 *    know... so Ill play some Sudokus and see if I can find bugs to fix.
 * </pre>
 * <hr>
 * <p>
 ...
 * 2. No timings for this release, coz its only a intermediate step.
 ...
 * 2. Its about 12 seconds slower than previous, which I think is acceptable
 ...
 * Config, and re-ordered the code in SudokuFrame. It was a dogs breakfast.
 * <pre>
 *        time(ns)  calls  time/call  elims      time/elim hinter
 ...
 * 3. Next I really dont know. I keep thinking I should have a look at Mutant
 *    Fish... so Ill do that, but Im not real hopeful. I suspect theyll be
 *    a bigger waste of time than theyre worth, but my predictive capacity is
 *    prodigously weak, so I shall look, at the very least.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.074 2020-06-27 14:12:14</b> Im releasing because its time to.
 * Fixed a few bugs. Ctrl-L now works properly, but valid does a brute-force
 * solve every time HdkAlsXyWing & HdkAlsXyChain hint. Im getting close the
 * point where valid is no longer required, so Im not too worried if its a
 ...
 * 3. Next I still dont know. I looked at Mutant (and Franked) Fish, but gave
 *    up. Its too complex for me. Ill think of something.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. Next Im still scratching me ead.
 * </pre>
 * <hr>
 * <p>
 ...
 * NOTES:
 * 1. 02:40 but dont get over excited. I dropped A5..10E completely to see how
 *    itd go. I was hoping for under 2 minutes with ALS-XZ and ALS-XY-Wing
 *    (but no ALS-XY-Chain).
 * 2. Release 6.30.076 2020-07-01 19:45:16
 *    => DiufSudoku_V6_30.076.2020-07-01.7z
 * 3. Next Im still scratching me ead. More profiling I guess.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.077 2020-07-10 17:24:17</b> Using HoDoKus "complex" Fish hinter
 * from Sudoku Explainer turned out to be pretty easy. Making it run faster is
 * the difficult part. Its really really really hard! Seriously complex s__t.
 * <pre>
 *        time(ns)  calls  time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. 05:29 is 3 MINUTES slower but yet get a bit of that on the big jobs. Its
 *    seriously complex, highly abstracted code, so making it faster is very
 *    challenging. Im releasing so I can clean-up the old logs.
 ...
 * <hr>
 * <p>
 * <b>KRC 6.30.078 2020-07-20 11:42:18</b> Forgive me father its been 10 days since
 * my last release and I just need to rub one out. Actually I need to delete my
 * log-files and I want a backup before I do so. Ive been pissin about trying
 ...
 * NOTES:
 * 1. 04:11 is a minute faster, which is nice, but dont get too excited: Ive
 *    dropped the non-performant hinters to get it down that far. Ive been
 ...
 * (4K -> 75% hit rate) but now theres s__tloads of logs so I release to back
 * and clean them up.
 * <p>
 * <b>BROKEN:</b> I discovered that my desktop "release" SudokuExplainer doesnt
 * run: HoDoKu ClassNotFound, so can only run in situ now. I need a real
 * programmer to workout whats happened to the classpath. Blame Nutbeans!
 * <pre>
 *        time(ns)  calls  time/call  elims   time/elim hinter
 ...
 * NOTES:
 * 1. 03:20 is nearly minute faster, which is nice, but dont get too excited:
 *    Ive been pissin about with hinter selection again. I made improvements
 *    to the KRAKEN search, especially Ints CACHE_SIZE from half a K to 4K.
 ...
 * This release Ill pack HoDoKu and SudokuExplainer into a single 7z. No more
 * dumb mistakes like this. Its really annoying because at one stage I had
 * it finding a couple of thousand fish hints, (Ive got the logs to prove it)
 * and then I had to revert when I broke s__t I couldnt workout how to fix.
 * And now I cant get it back to its uber-hinty state. And Im REALY REALY
 ...
 * NOTES:
 * 1. 03:34 is 12 secs slower than prev, which I can account for because its
 *    unoptimised because I had to revert HoDoKu to the zip, coz Im a putz!
 ...
 * and it totally sucks bags. Its slow and doesnt work and I feel pretty
 * stupid right now. But there was a nice tender lesso kiss on the tele and
 * thats always nice. So maybe Ill have another go at it tomorrow.
 * <pre>
 *        time(ns)  calls  time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. 03:25 is 9 secs faster than prev, and thats about as fast as I know how
 *    to make it.
 * 2. Release 6.30.081 2020-08-06 22:26:21
 *    => DiufSudoku_V6_30.081.2020-08-06.7z (including HoDoKu from now on)
 * 3. Next I dont know. Maybe nothing. I havent made substantial advances in
 *    the war on Sudoku in months... so I really do think Im all in. I reckon
 ...
 *    Swampfish to achieve it. They were both over a second a hint. Thats my
 *    limit. Im really only releasing to take a back-up and clean-up logs.
 * 2. Release 6.30.082 2020-08-10 21:42:22
 *    => DiufSudoku_V6_30.082.2020-08-10.7z (including HoDoKu from now on)
 * 3. Next I dont know. Id still like to ship it on my birthday.
 * </pre>
 * <hr>
 * <p>
 ...
 *    hints to find about 100 elims whichll be found anyway. It wasnt intended
 *    to be a race-car, but I use it to find a puzzle which trips over a bug;
 *    and you can switch it off at will. It was on for this run. Ill switch it
 ...
 * 3. Next I dont know. I still havent shipped it. Dont think itll work on
 ...
 * 3. Next I dont know. I pre-shipped it this arvo, BEFORE I discovered the
 ...
 * NOTES:
 * 1. Last top1465: 4:46 is 1:22 slower, so I am worried; but Ive just fixed
 ...
 * 3. Next I look at why its now a minute and twenty-two seconds slower.
 *
 ...
 * release, which was rushed out because I dont control when we go to town. I
 * should NOT have released untested. That was a BIG mistake and I wont make it
 ...
 * NOTES:
 * 1. Last top1465 03:03 but no Krakens so dont get excited. Ran again with all
 *    Krakens for 4:04, so Krakens take 1:01. Ran again w/out Kraken Swordfish
 *    (2.285 s/e) and Kraken Jellyfish (20.567 s/e) overheated 4:37. Cant win!
 *    So Im not concerned by speed (or lack thereof) at the moment.
 ...
 * <hr>
 * <p>
 * <b>KRC 6.30.088 2020-09-07 11:20:28</b> Release coz Im at the library and I can.
 ...
 *    so we ignore it and apply the hint despite its predicate being outdated.
 * </pre>
 * <hr>
 * <p>
 ...
 * NOTES:
 * 1. Last top1465 03:26 is 13 second faster, with Directs. I dont know why.
 ...
 * 3. Next Im boosting FishSolver, to remove the HoDoKu dependancy.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.091 2020-10-06 18:06:31</b> Boost HoDoKuFisherman does Finned, Franken,
 * and Mutant fish. It can do Basic but doesnt coz Fisherman is faster. Its
 ...
 *   716,097,977,000   4107 174,360,354     11 65,099,816,090 Mutant Jellyfish  == Thats nearly 12 minutes.
 ...
 * 3. Next Im boosting Kraken into KrakenFisherman.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.092 2020-10-11 18:46:32</b> Boost Kraken Fish from HoDoKuFisherman into
 * KrakenFisherman. It works, but its so slow it wont even fit on one line.
 * <pre>
 *          time(ns)  calls     time/call  elims       time/elim hinter
 ...
 *    Dont panic, just dont use Kraken Swordfish at 21 seconds per elim,
 *    or Kraken Jellyfish at just 7 minutes and 39 seconds per elimination.
 *    Theres a second version called KrakenFisherman1 thats faster, but finds
 ...
 * 2. All I can say about Existing Hints is "dont use it". The code is far
 *    too complex. Its so complex I cant understand why it doesnt work.
 ...
 * 4. Next Im trying again to make KrakenFisherman servicably quick.
 * </pre>
 * <hr>
 * <p>
 ...
 *    excluding KrakenJellyfish, so dont get excited. KrakenSwordfish is down
 ...
 *    still need to clean-up "the mess" Ive made out of ALSs package, to get
 *    rid of the HoDoKu types, especially HdkAls. I also need to make Franken,
 *    Mutant, and Kraken fish faster; but Ill be buggered if I know how.
 * </pre>
 * <hr>
 * <p>
 ...
 * the code is not quite so mental. I probably havent eradicated all of the
 ...
 * 3. Next I need to clean-up "the mess" Ive made out of ALSs package, to get
 *    rid of the HoDoKu types, especially HdkAls.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.095 2020-10-17 22:25:35</b> test-cases for Idx, HdkSet and HdkSetBase.
 * Ive given-up on cleaning-up "the mess" Ive made out of ALSs package, to
 * get rid of the HoDoKu types, especially HdkAls; Ive tried 4 times and it
 * just fails like a bastard, so I feel totally inadequate, but none the less
 * I have given-up on it. So its past time to ship it again.
 * <pre>
 *       time(ns)  calls  time/call  elims time/elim hinter
 ...
 *    funky stuff is enabled, so were producing a solution, but its nowhere
 *    near as simple as the simplest possible solution.
 * 2. Release 6.30.095 2020-10-17 22:25:35
 *    => DiufSudoku_V6_30.095.2020-10-17.7z
 * 3. Next, I dont know. I really want the complex fish to be fast enough for
 *    practical use, but havent got a clue how.
 * </pre>
 * <hr>
 * <p>
 ...
 * implement, because I dont want to tempt fate, and also because I just dont
 * like them, mainly because I didnt invent them, that and I am too stupid to
 ...
 * </pre>
 * <p>
 * I simply havent invested the time required to understand them in order
 ...
 * It isnt, because it cant be! Its an oximoron ya muppets! It simply can
 * not be, or logic disappears up an eleph... oh wait, can I bum a lift. This
 * planets overrun by half-wits. Conservative. Tub-mulcher. Out.
 * <p>
 ...
 * <p>
 * How hards that sheep? Thats a goat ya muppet! Sigh.
 * <pre>
 *          time(ns)  calls     time/call  elims       time/elim hinter
 ...
 * NOTES:
 * 1. Last top1465 132:54 is expected with the big fish. Theyre all too slow.
 ...
 * from the als package; so theyre gone now, except HdkAls becomes Als and the
 ...
 * 3. Next, I dont know. I still really want to make complex fish fast enough
 *    for practical use, but havent got a clue how.
 * </pre>
 * <hr>
 * <p>
 ...
 * now loads an invalid grid, doesnt "fix" it, and doesnt complain; just does
 * the best it can with what its given. Im a bit proud of Idx.untilFalse: I
 ...
 * NOTES:
 * 1. Last top1465 02:37 with none of the big fish. Shell do.
 * 2. Release 6.30.098 2020-10-23 23:20:39
 *    => DiufSudoku_V6_30.098.2020-10-23.7z
 * 3. Next, I dont know. I still really want to make complex fish fast enough
 *    for practical use, but havent got a clue how. Ill ship this release
 *    tomorrow.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2020-10-24 06:10</b> Heres da hinter summary of the overnight Big Fish run.
 * <pre>
 *          time(ns)  calls     time/call  elims       time/elim hinter
 ...
 *    So now it just needs to get 1 hour and 19 minutes faster, so Ill write
 ...
 *
 * 2020-10-29 21:12 FYI I fixed the performance issues, and Im chuffed!
 * 1465 puzzles in   249,098 ms (00:04:09.098) at   170.033 ms per puzzle
 * Thats 27.832 times faster!
 *
 ...
 * <hr>
 * <p>
 * <b>KRC 2020-11-30 07:01</b> Disabled mutant hinters by commenting out Techs.
 * <pre>
          time(ns)	  calls	     time/call	  elims	     time/elim	hinter
 ...
 * 3. Next I do not know, I guess Ill just have a poke around and try to find
 ...
 * all Aligned*Exclusions boiler-plate code down into one complex succinct
 ...
 * 3. Next I do not know: Theres nothing else to steal from Sukaku, as far as
 ...
 * take over a day on its own. Although it looks like Franken Fish is rooted!
 * <pre>
 *          time(ns)  calls   time/call  elims      time/elim hinter
 ...
 * 2. Im working on boosting hinters from Sukaku. VWXYZ-Wing is new. My own
 *    invention based on WXYZ-Wing and UVWXYZ-Wing. I just figured there was
 *    one missing, and I was right. Its nice to guess right occassionally.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.104 2021-01-06 19:06:44</b> Im releasing just to clean-up the logs.
 * Ive implemented SueDeCoq in the als package, and also tried but failed at
 * DeathBlossom.
 * <pre>
 *        time(ns)  calls  time/call  elims   time/elim hinter
 ...
 * NOTES:
 * 1. Last top1465 06:07 isnt as bad as it sounds. Its a hot box.
 * 2. Release 6.30.104 2021-01-06 19:06:44 =>
 *    DiufSudoku_V6_30.104.2021-01-06.7z
 * 3. I dont really know what to do next. Ive tried and failed DeathBlossom
 *    and theres nothing else worth boosting in Sukaku, so I suppose I should
 ...
 * 3. I dont know what to do next. DeathBlossom is dead. Im sick of it. Ill
 ...
 * minute slower, but thats still OK coz I have no love for DeathBlossom, so
 * I wont be using it.
 * <pre>
 *        time(ns)  calls  time/call  elims     time/elim hinter
 ...
 * NOTES:
 * 1. Last top1465 4:10 is a minute slower, but thats still OK coz I no use!
 ...
 * NOTES:
 * 1. Last top1465 3:24. DeathBlossom takes half the time. Thats better!
 ...
 * isnt competitive with the old align.Aligned*Exclusion classes but its
 * getting there, and its just SOOOO much more succinct.
 * <pre>
 ...
 * its a hot afternoon so Im blaming HOT BOX SYNDROME, and releasing anyway.
 ...
 * NOTES:
 * 1. Last top1465 5:28 isnt too bad considering Im using the BIG excluders.
 ...
 * NOTES:
 * 1. top1465 36:49 not too bad with Jelly Mutant and Kraken, but theyre both
 ...
 * 3. I give-up on ComplexFisherman and KrakenFisherman. Theyre just too slow!
 *    So I really dont know what Ill tackle next... maybe nothing... making
 *    this faster has ceased to be fun coz 90% of attempts are failures; so Im
 ...
 * <pre>
 * NOTES:
 * 1. Theres no "serious" code changes since last release, so no log posted.
 * 2. Release 6.30.111 2021-02-23 08:33:50 =>
 *    DiufSudoku_V6_30.111.2021-02-23.7z
 * 3. Next I dont know. Ill keep java-docing and see if anything grabs me.
 * </pre>
 * <hr>
 * <p>
 ...
 * complex techniques theyre pinching hints off, but they ARE simpler, and my
 ...
 * NOTES:
 * 1. Last top1465 run took 03:26 so were back on track at a minute faster
 *    than my previous blow-out.
 * 2. Release 6.30.113 2021-03-11 16:38:53 =>
 *    DiufSudoku_V6_30.113.2021-03-11.7z
 * 3. Next I dont know. Ive looked at everything on sudopedia. All solving
 *    techniques (or equivalent) are now implemented in Sudoku Explainer, so
 *    one could now call it a "complete solution". So I think Ive achieved my
 ...
 * 3. Next I dont know. Ive implemented all sudopedia solving techniques. I
 *    guess Im finally done.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.115 2021-03-24 08:53:55</b> GEM Mark 2 now finds more hints (75550)
 * than Mark 1 (51224). Ill actually release this build, I think, today.
 * <pre>
 *        time(ns)  calls  time/call  elims   time/elim hinter
 ...
 * 3. Next I dont know. Ive implemented all sudopedia solving techniques, so
 *    I guess SE is finally complete. I think Ive achieved my goal: To explain
 *    any/all Sudoku puzzles, as simply and quickly as possible, using logic.
 *    Well its as fast as Im able to make it. Somebody else could possibly
 *    get it running faster, but I can gaurantee you theyll be trying. I can
 *    dance by the record machine.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.116 2021-03-25 13:28:55</b> GEM mark 3, with its own Multi hint.
 ...
 * 3. Next I dont know. Ive implemented all sudopedia solving techniques, so
 *    SE is complete. Ive achieved my goal: To explain any Sudoku puzzle, as
 *    simply and quickly as possible. Well its as fast as Im able to make it.
 *    Somebody else might make it faster, but theyll certainly be trying.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. Next I dont know. I might take a week off, or atleast a day.
 * </pre>
 * <hr>
 * <p>
 ...
 * NOTES:
 * 1. Last top1465 run took 03:18, 4 seconds slower, but Im not worried,
 ...
 * 3. Next I dont know. I need some time off to think.
 * </pre>
 * <hr>
 * <p>
 ...
 *    but anything under 4 minutes feels "pretty snappy" in the GUI, so Im not
 ...
 * <p>
 * GEMs test-cases are still broken. Hint-types changed coz of "upgrades".
 * Ill deal with it later: find some new "simple type" hints.
 * <pre>
 *        time(ns)  calls  time/call  elims   time/elim hinter
 ...
 * NOTES:
 * 1. Last top1465 run took 03:12, 9 seconds faster. Thats better.
 ...
 *    But if you look at the Naked Singles time, its nearly double, so I blame
 ...
 * 3. Next I dont know. I really need a break!
 * </pre>
 * <hr>
 * <p>
 ...
 *    thing thats changed, and it took about the same time, so not worried.
 *    Im blaming the bloody JIT compiler, or maybe its just happenstance.
 ...
 * which is up from 185959 previously, and is as far as Id ever want to go.
 * There are other usable hinters, but theyre all too slow to warrant. SE will
 * still solve the puzzle, without GEM being so bloody greedy. Im only doing
 * this because Im an ornery old git who cant keep it simple.
 * <pre>
 *        time(ns) calls  time/call  elims   time/elim hinter
 ...
 * 3. GEM is finito. Next I dont know. I still need a break!
 * </pre>
 * <hr>
 * <p>
 ...
 * constraints on gemSolves hinters, so GEM is now uber-greedy, and still
 * relatively fast. Im building to clean-up the logs and create a back-up.
 * <pre>
 *       time(ns) calls  time/call  elims  time/elim hinter
 ...
 * 3. GEM is polished. Next I dont know. I still need a break!
 * </pre>
 * <hr>
 * <p>
 ...
 *    Thats fine: anything under 4 minutes feels "pretty snappy" in the GUI.
 * 2. Release 6.30.127 2021-04-24 08:18:07 =>
 *    DiufSudoku_V6_30.127.2021-04-24.7z
 * 3. Im done with Sudoku Explainer, I guess. I dont think there are any
 *    unimplemented hinters that I can understand well enough to do them any
 *    sort of justice, so thats it from me I suppose. Good luck with it.
 *    From here on out Ill fix reported bugs, probably for a couple of years,
 *    but other than that Ill find something else to dedicate myself to; after
 *    I take a good long break. Ive been working on this (on and off) for a
 *    decade, so Ive got the s__ts with it. sigh.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.128 2021-05-04 14:37:08</b> No real changes, just cleaning up crap, and
 * building to release because Im going shopping this afternoon.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 *    hints-tree and itll apply the current hint and get the next one.
 * </pre>
 * <hr>
 * <p>
 ...
 * Techs in this Difficulty range. The Tech difficulty (a base for hints of
 * this type) is displayed in the TechSelectDialog. The user can look at
 * TechSelect and Generate side-by-side to see which techs generate Difficulty
 ...
 * me, so I gave up. I still think its possible for each control event-handler
 * to delegate to a "generic" form-level event handler, but I cant see HOW.
 * <pre>
 *        time(ns)  calls  time/call  elims   time/elim hinter
 ...
 * NOTES:
 * 1. Last top1465 run took 02:53, 39 secs slower, but Im using STUVWXYZ-Wing,
 *    FinnedJellyfish, DeathBlossom, and Sue De Coq; so its all bloody good.
 *    Anything under about 5 minutes feels "pretty snappy" in the GUI.
 * 2. A Non-release build, to back-up and remove the log-files.
 * 3. Next I really dont know.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.131 2021-05-19 08:07:11</b> Building for release because were going
 * shopping today. Ive been pissing-about with BasicFisherman to make it
 ...
 *    using STUVWXYZ-Wing, FinnedJellyfish, and Sue De Coq. Its all good.
 ...
 * 3. Next I really dont know, bit Ill think of something.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.132 2021-05-25 09:24:12</b> Release coz were shopping today. Ive been
 * updating comments, hint explanations, and documentation. Still no comprende
 * why BigWing swaps x and z. Its nonsensical.
 * <pre>
 *        time(ns)  calls   time/call  elims      time/elim hinter
 ...
 * 3. Next I dont know. Ill poke around to find something to improve.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. Next I dont know. Just pissin about finding hinters to make faster.
 * </pre>
 * <hr>
 * <p>
 ...
 * 3. Next I dont know, but Ill find something to do.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.135 2021-06-04 16:04:15</b> Speed-up AAlsHinter by introducing new
 * AlsFinder and RccFinder classes. Its actually a second slower, but its
 * cleaner, so Im keeping it anyway. sigh.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * <hr>
 * <p>
 * <b>KRC 6.30.137 2021-06-09 15:28:17</b> Cleaning-up messy crap, and stuff I dont
 * use any longer.
 * <p>
 * Heres the release README:
 * <pre>
 * 1. LogViewHintRegexDialog: the Log-view gets its hint-regex from a custom
 *    dialog-box. The contents of combo-boxs drop-down list is stored in the
 ...
 *    top of this list. Its just a text-file, so you can maintain it using
 *    your text-file editor (Netbeans, vi, or whatever).
 *
 * 2. Its now BigWings OR S/T/U/V/WXYZWings in LogicalSolver.configureHinters.
 *
 ...
 *    cant increment a "global" static, so all of grids caches were "dirty".
 *    However, gemSolve can increment the grid.attribute, because it can just
 *    throw-away its own copy of the grid when its finished with it. Simples.
 *
 ...
 *    Im keeping hintNumber in Grid though, because its a better design.
 *
 ...
 *    constructor doesnt need to create unwanted hinters. Downside is that
 *    Config references Tech and now Tech references all the individual
 ...
 *    I still cant use them, because theyre still too bloody slow. sigh.
 *
 ...
 *    not why so many (a dozen) are being retained. Ive cleaned-up all hint
 ...
 *    memory after GC and shows not just whats retained but da reference chain
 *    that retains it, and such a tool doesnt exist, AFAIK.
 *
 ...
 *    techs are also enforced in LogicalSolver.constructor so that you cant
 *    even hack-around it by changing the registry manually. I might want to
 *    rip that out: its nice to be able to hack around problems by changing
 ...
 *    where-as recursiveSolves loop eats UnsolvableException and just guesses
 ...
 *    it can only be used for debugging. Its too verbose for "normal use".
 *
 ...
 *    a better feeling for the actual complexity of solving this puzzle. Its
 *    inspired by hobiwans approach. I used max-difficulty, because thats what
 *    made sense to me. Humans put more weigth on "how hard is it overall"
 *    verses "how hard is its hardest".
 ...
 *    a GEMHintBig doesnt have a far lower difficulty than setting all its
 ...
 * NOTES:
 * 1. Last top1465 run took 01:47 which is 14 seconds faster, but dont get
 *    excited, its only faster for dropping slowish hinters.
 ...
 *    old s__t out. Basically its a s__tfest.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.138 2021-06-13 11:43:18</b> Still cleaning-up my own mess.
 * <p>
 * Heres the additions to README.txt:
 * <pre>
 ...
 * Cell arrays on the fly, because Javas new array (and GC) is a bit slow.
 * </pre>
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * <hr>
 * <p>
 * <b>KRC 6.30.139</b> is MIA. I dont know how I stuffed-up, but stuff-up I did.
 * <hr>
 * <p>
 ...
 *    your pants on, thats only because Grid.AUTOSOLVE = true, so its faster
 ...
 * 3. Next I really dont know, but Ill think of something. This is ultimate
 *    ninja-level fast, but Knuth is still 100 times faster, and even solves
 *    invalid Sudokus. sigh.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.141 2021-06-22 16:02:21</b> LogicalSolver hinter summaries in
 * LogicalSolverTester log now in wantedHinters order. Replace keySet+get with
 * entrySet on all HashMaps. IMySet.visit. Split Frmu out of Frmt.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 *    were not sorting hinters every time we print a summary, and not finding
 *    the Grid in AHint.apply. So yeah, cop that one up ya!
 *
 *    As a baseline HoDoKu took 1465 in 165,680 ms (02:45.680) at 113.092 msp
 *    with my fix to its Nets (Dynamic Chaining), and now SE is nearly twice
 ...
 *    who (trust me on this) was a freeken genius, is totally amazing. Im a
 *    bit bloody chuffed! Woo-____ing-hoo!
 *
 *    Juillerat took 17:48. Ill bet that SudokuMonster would fail to finish.
 ...
 * how little work was required in comparison to the months Ive spent getting
 * no-where fast on the nominally "slow bits". Sigh. Java aint MASM. Sigh.
 * Its more of an ASSMASM. lols.
 * <p>
 ...
 * NOTES:
 * 1. Last top1465 run took 1:35, which is 4 seconds faster again, which Im
 *    pretty stoked about. Maybe I should spend some time optimising the bits
 *    that Ive previously dismissed because at-face-value they dont look at
 ...
 * a bloody nightmare. I should port it to Logging, but Im too lazy.
 ...
 * 2. Build 6.30.148 is releasable. Ill ship it as soon as I can.
 *    as DiufSudoku_V6_30.148.2021-07-09.7z
 * 3. Next I think I give up. Im struggling for motivation. Well see.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.149 2021-07-09 17:45:29</b> Config written-to/read-from file instead of
 ...
 * 2. Build 6.30.149 is releasable. Ill ship it as soon as I can.
 ...
 * NOTES:
 * 1. 1:39 is 6 seconds slower than previous, but Im not worried coz I was
 *    running Windows Media Player at the time, which is a CPU hog. So all in
 *    all its just another prick Ive ignored. Careful with that axe Eugeene.
 ...
 * NOTES:
 * 1. 1:32 is 7 seconds faster than previous, but dont get excited, he was a
 ...
 * a few ancillaries I tried it again and its fixed, so Im building it for
 ...
 * HintCache, and likewise both of its implementations, to better reflect
 * there current status.
 * <p>
 * During GUI testing for this release Ive discovered that the "Stop" button
 * is not working, and Ill be ____ed if I know why, but Im building anyway.
 * It also appears to be MUCH better at generating IDKFAs: what used to take
 * thousands of puzzles now seems to take dozens, and I know not why. Maybe my
 * hinter selection is different, or one of the hinters isnt working? Anyway,
 * theres MUCH less motivation for the user to press "Stop", so Im past
 ...
 * NOTES:
 * 1. 1:31 is same as previous. I hoped it might be a bit faster, but its
 *    cleaner, so Ill take it anyway.
 * 2. Build 6.30.154 2021-07-18 11:08:33 is releasable. Ship it!
 *    as DiufSudoku_V6_30.154.2021-07-18.7z
 * 3. Next I try to work-out why the bastard now wont stop in java.exe, when
 *    it did stop in the bloody IDE! Im not sure it ever stopped in java.exe,
 ...
 * type thats thrown, not the type of the actual exception. Oops!
 * <pre>
 * NOTES:
 * 1. No need to re-run the batch. Its the GUI that matters. Generate stops!
 * 2. Build 6.30.155 2021-07-18 17:56:35 is releasable. Ship it ASAP!
 *    as DiufSudoku_V6_30.155.2021-07-18.7z
 * 3. Next Im gonna needs me a stem gun, and a sianide pill. Vielette was HOT!
 * </pre>
 * <hr>
 * <p>
 ...
 * except for the stuff that doesnt. sigh.
 * <p>
 * We still have problems solving an IDKFA (and presumably other Difficulty,
 * its just MUCH harder to see) while actively solving the generated IDKFA, so
 * its better to wait for it to generate the replacement before solving the
 ...
 * with the current puzzle, but that doesnt seem achievable coz of hinters
 * static statefull variables.
 * <p>
 * The ONLY practical answer is moving the factory to its own JVM, and have it
 ...
 * Difficulty stored in its own cache-file. I havent done this, I would just
 ...
 * 3. Next Im gonna have a chook around for something to tidy-up.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.157 2021-07-20 08:51:37</b> jar-only-distribution rolls its own Config.
 ...
 * Config creates a default-file from defaultConfig(), if file !exist.
 * NOTES:
 ...
 * 3. Next I dont know. The story of my life really.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.158 2021-07-28 11:22:38</b> Im building for a back-up before I change
 * AlsChain for speed, ie rewrite RccFinder "properly" in the OO-sense.
 * <pre>
 * 1. io.IO.load is now gui.GridLoader.load coz its only used in the GUI, and
 ...
 *    Text (.txt) formats still implemented, even-though theyre deprecated.
 *
 *    Ill write a tool that rewrites *.txt files in-or-under a directory, and
 ...
 *    a VERY large array of 134 million ints, but its no faster. I presume
 ...
 *    the GC runs all-the-time, so no, its no faster. sigh.
 *    I wish the bit-twiddling in Integer, Long and StrictMath (et al) was
 *    implemented in firmware! I also wish that long math wasnt slower than
 *    int math in Java. Paying for 7 64-bit cores and then using bloody half of
 *    them, because its faster, really ____ing s__ts me!
 *
 * 4. Grid.otherCommonRegion moved house to Regions.otherCommon. Its used only
 ...
 *    thats how they should be stored; then to get the common regions you just
 *    and-together all of the cell.ribs (RegionIndexBitS). Food for thought,
 *    but I havent done this because my callers arent performance critical.
 *    Im called by two methods:
 ...
 *      in the GUI when a putz wants Direct*, so I dont care.
 *    so performance of Regions.otherCommon is NOT important! That is WHY my
 *    Idx.FIRST idea was a dismal failure: everything Ive done so far presumes
 *    its calling Integer.numberOfTrailingZeros, so its always the last-thing
 ...
 *    it up a bit, so now its even more of a brain-bender. Theres some clever
 *    ideas buried in all that obfuscation. Its still a pig though.
 *
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * Theres two abstract classes: RccFinderAbstract is "base", defining no-op
 ...
 * ASAT 2023-07-31 NoOverlaps not used so dropped, leaving two implementations:
 * 1. RccFinderForwardOnly extends RccFinderAbstract
 * 3. RccFinderAll         extends RccFinderAbstractIndexed
 * </pre>
 ...
 * 3. Next I still need to speed-up RccFinder, but its really hard.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.160 2021-08-11 07:10:40</b> build to clean-up old logs. I honestly cant
 * recall what the hell Ive done, if anything, in the past two weeks. I just
 ...
 * but its only using ArrayList.contains, which is slow on a list-of-lists, so
 ...
 * {@code Set<Idx>} so I havent bothered. I tried, so I know its lots of work.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * 3. Next Ill just keep poking around looking for something to play with.
 *    Ive given-up any hope of enspeedonating the RccFinder. Its too hard!
 * </pre>
 * <hr>
 * <p>
 ...
 *    old BasicFisherman class, so theres really still no rush to ship, but I
 *    need to goto the library to complete the Census, so I may as well today.
 *    as DiufSudoku_V6_30.161.2021-08-12.7z
 * 3. Next I really dont know. I think Ill take a couple of days off and read
 *    my book: The Theory of Everything by Stephen Hawking. Its really quite
 ...
 *    and Im writing 1.3 off to a bad run.
 * 2. 6.30.162 2021-09-04 04:57:42 looks releasable, but theres no rush.
 *    DiufSudoku_V6_30.162.2021-09-04.7z
 * 3. Next Im still trying to cleanup after myself.
 * </pre>
 * <hr>
 * <p>
 ...
 * NOTES:
 * 1. 1:35 is three seconds slower again than last time, so were now up 5
 *    seconds, which has become a worry, so this build should not be released.
 * 2. 6.30.164 2021-09-21 09:28:44 should NOT be released, coz its slower.
 ...
 * but its not right. Its faster, but relies on the Validator to suppress
 ...
 * 2. 6.30.166 2021-10-07 09:00:44 should NOT be released, coz its slower.
 ...
 * 1.5 times as many hints. I also reordered the hinters but thats slower too.
 * sigh. Im just not was worried about performance as I was coz its all now
 ...
 *    three minutes is still OK. So ____ It Shell Do!
 * 2. DiufSudoku_V6_30.167.2021-10-10.LOGS.7z can be released, but its slower.
 ...
 * find 1.5 times as many hints, by caching related RCCs by previousAlsIndex
 ...
 * NOTES:
 * 1. 1:50 is 13 seconds faster, but were still WELL up on the best-ever valid
 *    run at 1:30, but anything under 3 about minutes feels snappy enough in
 *    GUI, and Im definately not as performance-centric as I was in the past.
 * 2. DiufSudoku_V6_30.168.2021-10-11.LOGS.7z can be released, but no rush.
 * 3. Next I dont know. Ill find something.
 * </pre>
 * <hr>
 * <p>
 ...
 * alone contain each other, so its a tad faster with this check.
 * <p>
 * <pre>
 ...
 * twice as long in wait-time, but I reckon its worth keeping anyway, despite
 ...
 * And AlsChain wouldnt be so bloody slow.
 * But its not an ideal world. sigh.
 *
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * NOTES:
 * 1. 1:49 is a whole second faster than previous. Gee wow! Were still WELL up
 ...
 * 3. Next I dont know. Ill find something. AlsChains are starting to REALLY
 *    shit me, so I reckon leave it for a month or two, then come back to it.
 *    Its amazing how much you dont see when youve been staring straight at
 ...
 *    the available 90 seconds, which Ive just done. Its worth noting that
 ...
 * 3. Next I dont know. I tap-out on AlsChain. It officially just s__ts me.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.171 2021-10-18 19:02:51</b> in-lined everything in AlsChain again, for
 * speed, and its about 20% faster than the nieve version.
 * <pre>
 *        time(ns)  calls  time/call  elims  time/elim hinter
 ...
 * NOTES:
 * 1. 03:08 aint great but ____ it, shell do.
 * 2. DiufSudoku_V6_30.171.2021-10-18.7z slower, but still releasable.
 * 3. Next I dont know. I really tap-out on AlsChain this time.
 * </pre>
 * <hr>
 * <p>
 ...
 *    were still over a minute slower than the fastest ever, which found 1090
 ...
 * 3. Next I dont know. I really really tap-out on AlsChain this time. sigh.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.173 2021-10-20 10:21:53</b> Updated AlsWing and AlsXz with learnings
 * from AlsChain. Theyre both just a little faster, and cleaner, IMHO.
 * <pre>
 *        time(ns)  calls  time/call  elims  time/elim hinter
 ...
 *    but the JIT compiler is muercurial, so Im hoping theyll bed-in nicely.
 * 2. DiufSudoku_V6_30.173.2021-10-20.7z faster, but still slow = Releasable.
 * 3. Next I dont know. I need to look for something else to play with for a
 ...
 *    AlsChains recalcitrance is a challenge, which I accept, just not now.
 ...
 *    the GUI, and three minutes is OK, so its all good.
 * 2. DiufSudoku_V6_30.174.2021-10-20.7z RELEASE THIS!
 * 3. Next I dont know. I shall see.
 * </pre>
 * <hr>
 * <p>
 ...
 * NOTES:
 * 1. 01:33 is 24 seconds faster than previous, but were only finding ~60% of
 ...
 * with the non-determinism of the TIMEOUT approach so Ive ripped-out da grow-
 ...
 * correctness. Id rather be consistently wrong than occassionaly right.
 ...
 * 3. Next I dont know. I shall see. I need to stop playing with AlsChain but
 *    its still the slowest hinter that I still use, so its still the s__t
 *    that most needs attention, but I really dont know what else to try.
 *    I found a @stretch in RccFinderAllAllowOverlaps but its a big one. Ive
 ...
 * coz its faster. RCC_CACHE is back for speed. TechSelectDialog selects one
 ...
 * 3. Next I dont know, but I promise to stop fiddling with AlsChain!
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.179 2021-10-27 17:56:59</b> AlsChain rccCache has its own IAS
 ...
 *    with 1697 elims verses ALS_Chain_6s 1720, so its a bit more accurate,
 *    and as Ive said repeatedly the GUI feels OK when batch is three minutes.
 *    But Oh wait, 1697 is less than 1720, so its NOT "more accurate", so calm
 *    down and think it through. The actual determinant is the number of times
 *    its called. 4390 is lower than 4487, which means ALS_Chain_7 is finding
 *    more effective hints, just (as youd expect) less of them. I dont really
 ...
 *    only thing that couldnt be faster is Voyager, and its out of my league.
 * 2. DiufSudoku_V6_30.179.2021-10-27.7z needs releasing, tomorrow, I promise.
 * 3. Next I dont know, but I need to stop pulling my AlsChain!
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.180 2021-10-28 10:10:10</b> AlsChain has ALS_HAS_ANY_RELATED_RCCS and an
 * alsCeiling, so its a tad faster. The hunter, is still endlessly seeking.
 * <pre>
 *        time(ns)  calls  time/call  elims  time/elim hinter
 ...
 *    and thats what Ive changed, so accepted. More-RAM-slow! More-RAM-slow!
 * 2. DiufSudoku_V6_30.180.2021-10-28.7z needs releasing, today!
 * 3. Next I dont know, but I still need to stop pulling my AlsChain!
 * </pre>
 * <hr>
 * <p>
 ...
 * growth problem. Im a bit proud of this, in fact Im chuffed.
 * <p>
 ...
 * 3. Next I dont know, but now I CAN stop pulling my AlsChain! Funny how s__t
 *    like AlsChain keeps bothering you until you fix it. Ive STILL got it for
 ...
 * 3. Next I dont know, but Ill find something.
 * </pre>
 * <hr>
 * <p>
 ...
 *    days work. Were will past the point of dimishing returns here. sigh.
 ...
 * 3. Next I dont know, but Ill find something. Im done with AlsChain.
 * </pre>
 * <hr>
 * <p>
 ...
 * all the springs dont come flying out of the box if/when a user wants some
 * piece-of-s__t-hinter that shouldve been written-off years ago. Yes BUG, Im
 * looking straight at you. Thats it: BUG has to go!
 * <pre>
 ...
 *    17:48, but a "little bit" more thorough. Ergo, I think its safe to say
 ...
 *    that all of the springs dont fly out of the box no matter which hinter/s
 ...
 * NOTES:
 * 1. 1:25 is same as previous, but AlsChain is 1.5 secs faster and thats what
 *    Ive been working on. The other change is the chainer cache now refreshes
 ...
 *    give-up now, so this is as good as its going to get on my watch.
 ...
 * 2. DiufSudoku_V6_30.188.2021-11-25.7z Ill release this one. No rush.
 ...
 * 2. DiufSudoku_V6_30.189.2021-11-28.7z Ill release this one. No rush.
 * 3. Next I seek more "my bads" by continueing to desk-check each hinter, but
 *    everything from-here-down is beyond my brain: I cant hold it all in my
 ...
 *    tool that found unused code. Ill update Java versions, and find a tool.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.192 2021-12-03 13:12:12</b> Removed Grid.Cell.hashCode using
 * Grid.Cell.i (indice) instead coz its a bit shorter, and more significant
 ...
 * NOTES:
 * 1. 1:25 is a second slower than previous. Meh. I dont care! Really. Sigh.
 ...
 * <hr>
 * <p>
 * <b>KRC 6.30.193</b> Its a bust! So it got the chop.
 * <hr>
 * <p>
 * <b>KRC 6.30.194 2021-12-04 08:00:14</b> Chainers use circular-arrays instead
 * of Queues. They make chainers about 10% faster, butre unmaintainable, so do
 ...
 * NOTES:
 * 1. 1:03 is fast, but thats with the hair-dryer, so ney come in ye pant.
 * 2. DiufSudoku_V6_30.195.2021-12-06.7z unreleasable: validating XColoring.
 ...
 *    have been "fixing" UniqueRectangle, whichs down about 100 elims. I think
 *    that what Ive done is correct, but Ive really made no attempt to verify
 *    that is in fact correct. Im just winging it. Sigh.
 ...
 *    is actually faster for not using the ALS-cache, but doesnt use ALS-cache
 *    so DeathBlossom has to prime the cache, and consequently takes longer,
 *    hence BigWings is preferred, because its faster overall. Sigh.
 * 2. DiufSudoku_V6_30.198.2021-12-18.7z is OK to release. No rush.
 * 3. Next I still seek "bad eggs" but not finding much. I reckon its about as
 ...
 * first which forces a rejig of Difficultys to reduce largest, so I went the
 ...
 * NOTES:
 * 1. 1:22 is a second under par, which is about what Id expect from putting
 ...
 *    in order to REALLY kick Sudokus ass. Please dont feed them after dark,
 *    never get them wet, and dont take them seriously. Nor me. Sigh smile.
 * </pre>
 * <hr>
 * <p>
 ...
 *    Id also hoped that isX/YChain fields would be faster. They arent, which
 ...
 * 3. Next I keep hunting my bads. Theres lots of silly code in SE. The more I
 ...
 *    hoped it would be half-a-second faster, but Ill stick with it, to let it
 ...
 *    the ones Ive changed) are slower so I blame hot box syndrome, where the
 *    CPU slows itself down instead of overheating, despite the fact that its
 ...
 * NOTES:
 * 1. 1:21 is a second under par, but DeathBlossom is what Ive changed and it
 *    was 7.042 and is now 4.653 which is about 2.4 seconds faster, so I know
 *    not whats going on with the rest of it, but itll come good again.
 ...
 * NOTES:
 * 1. 1:28 is 7 seconds over par, but Ive reinstated all the hinters that Id
 *    previously dropped for being "a bit slow". Theyre still a bit slow but
 *    theres no point pushing the batch below about 3:00. In the GUI Ill wait
 *    a few milliseconds for a more accurate answer to the question "Whats the
 *    next simplest available hint?". Basically, I wait for hinters that take
 *    upto 100ms per elimination in the batch. If I cant bring a hinter under
 ...
 * NOTES:
 * 1. 1:26 is a couple of seconds faster, but ____ it, shell have to do.
 ...
 * against the upgrade. You can run it in later VMs, but the music died some
 ...
 *    Im concreted onto 1.8 to achieve anything like it; the best I did in JDK
 ...
 * 3. Next nothing. No platform upgrade so SE can die with 1.8. Im disgusted
 *    with Oracle. Im sure they have reasons, which may be beyond me, but a
 ...
 * arent worth there salts. EmptyRectangle is a bit faster. Build for backup
 * pre split ridxs into parallel int[] places (.bits) and numPlaces (.size).
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 ...
 *    this is DEGREE_CEILING reduced to 6. Large ALSs arent worth there salts.
 *    Theres about a half a second from the other eight days work. Gee wow!
 * 2. DiufSudoku_V6_30.207.2022-01-19.7z is OK to release.
 * 3. Next nothing. Bail coz JVM is shot: C#, GNUCC, or the full MASM?
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.208 2022-01-22 11:19:28</b> NakedSet uses new knownNakedSets to
 * avoid reprocessing the same nakedSet repeatedly when it still wont hint. I
 * am only building to back-up the logs which are now too numerous.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 *     17,278,200 106375        162  56198        307 NakedSingle
 *     17,725,800  50177        353  16468      1,076 HiddenSingle
 *     74,118,000  33709      2,198  24955      2,970 Locking
 *     71,423,400  22070      3,236   3235     22,078 NakedPair
 *     66,702,800  21001      3,176  10228      6,521 HiddenPair
 *    110,322,800  18354      6,010   2250     49,032 NakedTriple
 *     98,594,700  17861      5,520   1435     68,707 HiddenTriple
 *     42,074,400  17521      2,401    682     61,692 Swampfish
 *     45,202,400  17259      2,619   1131     39,966 TwoStringKite
 *     93,825,800  16128      5,817   1036     90,565 W_Wing
 *     77,530,700  15421      5,027    532    145,734 XY_Wing
 *     45,666,200  15065      3,031    344    132,750 Skyscraper
 *    101,330,000  14885      6,807    575    176,226 EmptyRectangle
 *     63,721,800  14310      4,452    261    244,144 XYZ_Wing
 *     51,505,800  14067      3,661    232    222,007 Swordfish
 *    144,173,900  14006     10,293     94  1,533,764 NakedQuad
 *     75,318,600  13990      5,383     18  4,184,366 Jellyfish
 *    121,641,600  13986      8,697    207    587,640 HiddenQuad
 *    285,498,100  13925     20,502    231  1,235,922 Coloring
 *    855,421,400  13790     62,032    695  1,230,822 XColoring
 *  1,940,040,600  13536    143,324 129756     14,951 GEM
 *  1,376,764,600  12174    113,090    896  1,536,567 UniqueRectangle
 *  2,063,088,800  11538    178,808   2070    996,661 BigWings
 *  1,251,618,800  10398    120,371    217  5,767,828 FinnedSwampfish
 *  2,764,839,700  10221    270,505    271 10,202,360 FinnedSwordfish
 *  3,130,644,800  10021    312,408   2253  1,389,544 DeathBlossom
 *  5,151,091,100   8121    634,292   2055  2,506,613 ALS_XZ
 *  3,797,728,700   6830    556,036   2609  1,455,626 ALS_Wing
 *  7,782,755,700   4778  1,628,873   1876  4,148,590 ALS_Chain
 *    861,992,400   3325    259,245     15 57,466,160 SueDeCoq
 *  5,570,351,100   3322  1,676,806    718  7,758,149 UnaryChain
 *  4,012,231,100   2982  1,345,483    193 20,788,762 NishioChain
 *  4,779,845,500   2798  1,708,307   3644  1,311,702 MultipleChain
 *  6,199,700,200   1296  4,783,719   6312    982,208 DynamicChain
 *    100,908,900      6 16,818,150     24  4,204,537 DynamicPlus
 * 53,242,678,400
 * pzls      total (ns) (mm:ss)   each (ns)
 * 1465  70,004,827,200 (01:10)  47,784,864
 * NOTES:
 * 1. 01:10 is 3 seconds faster than previous, so I guess you can go ahead and
 *    wet ya pants finally. It was wasting time repeatedly claiming NakedSets
 *    when there can be no new elims, so now we check each NakedSet ONCE only.
 * 2. DiufSudoku_V6_30.208.2022-01-22.7z is BAD! DO NOT RELEASE!!!! It broke
 *    LogicalAnalyserTester, so I reverted it all out. Sigh.
 * 3. Next Java has gone to pot, so DO NOT USE JAVA! Maybe I should start again
 *    in another language, but thats just too much work! Sigh.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.209 2023-03-26 11:13:29</b>
 * Inlined code in WWing for speed, which, for reasons I do NOT understand,
 * made it run MUCH slower, but Im inclined to accept these changes anyway
 * because WWing itself was faster. Im gambling that my slowness is the JIT
 * compiler, buggering it all up. Ive noticed that hinters now run in a
 * different order than they did previously. I do NOT remember making such
 * changes, but it all makes sense logically, probably more than the previous
 * order, which was optimised for speed, not always running the "easier"
 * hinter first, as this version (more so) does. I ran the batch twice more,
 * but it was slower both times, presumably coz my CPU is now "pretty warm".
 * Not to worry too much. This looks OK and performance is NOT everything.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 *     24,021,500 106035        226  56552        424 NakedSingle
 *     20,645,800  49483        417  16115      1,281 HiddenSingle
 *     79,980,100  33368      2,396  24094      3,319 Locking
 *     87,578,000  22125      3,958   7845     11,163 NakedPair
 *     67,735,200  20095      3,370   8253      8,207 HiddenPair
 *    114,050,300  18028      6,326   1644     69,373 NakedTriple
 *    101,336,900  17635      5,746    962    105,339 HiddenTriple
 *     60,370,800  17449      3,459   1321     45,700 TwoStringKite
 *     46,948,600  16128      2,910    426    110,207 Swampfish
 *     85,590,400  15944      5,368    913     93,746 W_Wing
 *     79,303,900  15307      5,180    531    149,348 XY_Wing
 *     48,761,600  14952      3,261    336    145,123 Skyscraper
 *    100,711,900  14776      6,815    567    177,622 EmptyRectangle
 *     60,339,100  14209      4,246    261    231,184 XYZ_Wing
 *     56,408,700  13965      4,039    222    254,093 Swordfish
 *    151,960,200  13906     10,927     94  1,616,597 NakedQuad
 *     78,653,000  13890      5,662     18  4,369,611 Jellyfish
 *    121,701,600  13886      8,764     12 10,141,800 HiddenQuad
 *    326,114,700  13884     23,488    165  1,976,452 Coloring
 *    927,237,000  13768     67,347    695  1,334,153 XColoring
 *  1,939,924,200  13514    143,549 129663     14,961 GEM
 *  1,273,257,400  12171    104,614    896  1,421,046 UniqueRectangle
 *  2,180,358,700  11535    189,021   2059  1,058,940 BigWings
 *  1,461,780,000  10397    140,596    217  6,736,313 FinnedSwampfish
 *  3,347,750,000  10220    327,568    271 12,353,321 FinnedSwordfish
 *  3,098,664,100  10020    309,247   2257  1,372,912 DeathBlossom
 *  5,286,911,300   8119    651,177   2048  2,581,499 ALS_XZ
 *  4,426,982,200   6829    648,262   2614  1,693,566 ALS_Wing
 *  7,632,072,300   4773  1,599,009   1873  4,074,784 ALS_Chain
 *  1,144,323,000   3323    344,364     21 54,491,571 SueDeCoq
 *  6,786,605,200   3319  2,044,774    736  9,220,930 UnaryChain
 *  5,356,277,500   2976  1,799,824    193 27,752,733 NishioChain
 *  6,227,141,400   2792  2,230,351   3627  1,716,884 MultipleChain
 *  7,848,102,300   1298  6,046,303   6290  1,247,711 DynamicChain
 *    139,459,400      3 46,486,466     30  4,648,646 DynamicPlus
 * 60,789,058,300
 * pzls       total (ns)  (mm:ss)   each (ns)
 * 1465  101,757,439,700  (01:41)  69,459,003
 * NOTES:
 * 1. 01:41 is 31 seconds slower than previous, which is just plain ____ed.
 * 2. DiufSudoku_V6_30.209.2023-03-26.LOGS.7z.
 * 3. I wonder what OzMasmSudoku would look like?
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.210 2023-04-26 08:28:30</b> Various small ineffectual changes;
 * actually I mostly just improved comments and comments. Changed welcome.html
 * and improved explanations in some hints.
 * <pre>
 *       time(ns)  calls  time/call  elims  time/elim hinter
 *     24,548,900 106035        231  56552        434 NakedSingle
 *     20,449,800  49483        413  16115      1,268 HiddenSingle
 *     82,630,000  33368      2,476  24094      3,429 Locking
 *     89,207,500  22125      4,031   7845     11,371 NakedPair
 *     69,311,400  20095      3,449   8253      8,398 HiddenPair
 *    116,184,300  18028      6,444   1644     70,671 NakedTriple
 *    102,418,800  17635      5,807    962    106,464 HiddenTriple
 *     62,534,500  17449      3,583   1321     47,338 TwoStringKite
 *     47,769,600  16128      2,961    426    112,135 Swampfish
 *     87,495,500  15944      5,487    913     95,832 W_Wing
 *     83,196,500  15307      5,435    531    156,678 XY_Wing
 *     48,559,200  14952      3,247    336    144,521 Skyscraper
 *    101,204,400  14776      6,849    567    178,491 EmptyRectangle
 *     63,527,700  14209      4,470    261    243,401 XYZ_Wing
 *     55,977,200  13965      4,008    222    252,149 Swordfish
 *    149,091,400  13906     10,721     94  1,586,078 NakedQuad
 *     78,355,700  13890      5,641     18  4,353,094 Jellyfish
 *    121,262,400  13886      8,732     12 10,105,200 HiddenQuad
 *    336,977,200  13884     24,270    165  2,042,286 Coloring
 *    939,494,800  13768     68,237    695  1,351,791 XColoring
 *  2,003,077,800  13514    148,222 129663     15,448 GEM
 *  1,254,973,800  12171    103,111    896  1,400,640 UniqueRectangle
 *  2,161,112,100  11535    187,352   2059  1,049,593 BigWings
 *  1,783,160,900  10397    171,507    217  8,217,331 FinnedSwampfish
 *  3,564,940,800  10220    348,820    271 13,154,763 FinnedSwordfish
 *  3,135,443,200  10020    312,918   2257  1,389,208 DeathBlossom
 *  5,360,511,900   8119    660,242   2048  2,617,437 ALS_XZ
 *  4,403,699,700   6829    644,852   2614  1,684,659 ALS_Wing
 *  7,665,158,800   4773  1,605,941   1873  4,092,449 ALS_Chain
 *  1,210,902,500   3323    364,400     21 57,662,023 SueDeCoq
 *  6,873,806,500   3319  2,071,047    736  9,339,411 UnaryChain
 *  5,381,063,400   2976  1,808,153    193 27,881,157 NishioChain
 *  6,223,522,500   2792  2,229,055   3627  1,715,887 MultipleChain
 *  7,847,638,000   1298  6,045,946   6290  1,247,637 DynamicChain
 *    160,180,400      3 53,393,466     30  5,339,346 DynamicPlus
 * 61,709,389,100
 * pzls       total (ns) (mm:ss)   each (ns)
 * 1465  102,833,323,900 (01:42)  70,193,395
 * NOTES:
 * 1. 1.42 is about a second slower than previous. Meh! Im far less concerned
 *    about performance these days. Ive gone all soft in my old age.
 * 2. DiufSudoku_V6_30.210.2023-04-26.LOGS.7z.
 * 3. Isnt Sudoku just mad from the outset? Whats the point? Why bother?
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.211 2023-05-21 15:50:31</b> Superfisch is Fish AFAP.
 * <pre>
 * Superfisch: withoutFins=false withFins=true minSize=2 maxSize=3 maxFins=5
 * maxEndofins=3 maxFishType=1 maxKrakenType=-1 maxKrakenSize=0
 *
 *        time(ns)  calls  time/call  elims   time/elim hinter
 *      22,886,800 108111        211  57708         396 NakedSingle
 *      21,633,300  50403        429  16270       1,329 HiddenSingle
 *      82,422,800  34133      2,414  24138       3,414 Locking
 *      93,655,700  22798      4,108   7861      11,913 NakedPair
 *      73,066,400  20735      3,523   8211       8,898 HiddenPair
 *     123,116,700  18676      6,592   1601      76,899 NakedTriple
 *     114,233,700  18278      6,249    993     115,038 HiddenTriple
 *      63,440,800  18078      3,509   1333      47,592 TwoStringKite
 *      54,077,800  16745      3,229    404     133,855 Swampfish
 *      57,678,900  16577      3,479    905      63,733 W_Wing
 *      80,317,500  15947      5,036    538     149,289 XY_Wing
 *      51,167,700  15588      3,282    328     155,999 Skyscraper
 *     103,740,100  15418      6,728    580     178,862 EmptyRectangle
 *      61,562,900  14838      4,149    247     249,242 XYZ_Wing
 *      67,883,200  14606      4,647    226     300,368 Swordfish
 *     163,471,200  14547     11,237    100   1,634,712 NakedQuad
 *      99,474,300  14529      6,846     21   4,736,871 Jellyfish
 *     137,237,600  14525      9,448      1 137,237,600 HiddenQuad
 *     347,466,200  14524     23,923    202   1,720,129 Coloring
 *     943,156,400  14394     65,524    730   1,291,995 XColoring
 *   2,083,156,000  14135    147,375 119618      17,415 GEM
 *   1,411,220,100  12804    110,217    893   1,580,313 UniqueRectangle
 *   3,160,070,900  12170    259,660   2183   1,447,581 BigWings
 *   6,096,330,000  10965    555,980   2577   2,365,669 DeathBlossom
 *  10,789,876,000   8793  1,227,098   2109   5,116,110 ALS_XZ
 *   9,577,864,200   7455  1,284,757   2845   3,366,560 ALS_Wing
 *  15,635,740,600   5198  3,008,030   1908   8,194,832 ALS_Chain
 *   1,411,479,400   3709    380,555     24  58,811,641 SueDeCoq
 *  92,058,773,500   3705 24,847,172    347 265,299,059 Superfisch ============
 *   7,367,332,100   3456  2,131,751    525  14,033,013 UnaryChain
 *   6,127,234,400   3223  1,901,096     80  76,590,430 NishioChain
 *   7,485,078,900   3143  2,381,507   3394   2,205,385 MultipleChain
 *   1,921,815,000   1747  1,100,065   4153     462,753 DynamicChain
 *      17,867,000      6  2,977,833     24     744,458 DynamicPlus
 * 167,905,528,100
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  217,268,658,900 (03:37)  148,306,251
 * NOTES:
 * 1. 3:37 is almost two minutes longer than 1.42, so Superfisch is a bit faster
 *    than my disaster of seperate multiple Complex and KrakenFisherman; but it
 *    is still too slow seeking: Swampfish, Swordfish, and Jellyfish in Basic,
 *    Franken, and Mutant (not Krakens). But I dont care about performance.
 *    Note that Superfisch is a "hidden feature". It does not appear in the
 *    "Techniques Selection" dialog; so only hackers will ever see it.
 * 2. DiufSudoku_V6_30.211.2023-05-21.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-05-23 15:15</b> Superfisch Config Dialog to unhide Superfisch.
 * Only hackers will figure out how to use it still.
 * <pre>
 * ran 2023-05-23.15-02-56
 * Superfisch: withoutFins=false withFins=true minSize=2 maxSize=4 maxFins=7
 * maxEndofins=5 maxFishType=1 maxKrakenType=-1 maxKrakenSize=3
 *
 *        time(ns)  calls   time/call  elims     time/elim hinter
 *      21,159,500 108199         195  57759           366 NakedSingle
 *      21,117,300  50440         418  16276         1,297 HiddenSingle
 *      83,873,000  34164       2,455  24134         3,475 Locking
 *      96,027,900  22827       4,206   7859        12,218 NakedPair
 *      69,793,100  20766       3,360   8212         8,498 HiddenPair
 *     116,901,100  18706       6,249   1608        72,699 NakedTriple
 *     104,957,100  18308       5,732    993       105,696 HiddenTriple
 *      59,807,800  18108       3,302   1333        44,867 TwoStringKite
 *      50,392,100  16775       3,004    404       124,732 Swampfish
 *      55,047,200  16607       3,314    914        60,226 W_Wing
 *      77,074,900  15972       4,825    538       143,261 XY_Wing
 *      48,006,400  15613       3,074    327       146,808 Skyscraper
 *      95,946,400  15444       6,212    580       165,424 EmptyRectangle
 *      56,994,100  14864       3,834    249       228,891 XYZ_Wing
 *      63,737,200  14630       4,356    224       284,541 Swordfish
 *     147,708,400  14572      10,136    100     1,477,084 NakedQuad
 *      93,060,700  14554       6,394     16     5,816,293 Jellyfish
 *     124,912,700  14551       8,584      1   124,912,700 HiddenQuad
 *     318,463,600  14550      21,887    201     1,584,396 Coloring
 *     870,905,200  14420      60,395    746     1,167,433 XColoring
 *   1,940,821,000  14158     137,082 119014        16,307 GEM
 *   1,298,822,000  12831     101,225    892     1,456,078 UniqueRectangle
 *   2,886,172,900  12198     236,610   2186     1,320,298 BigWings
 *   5,619,834,700  10991     511,312   2578     2,179,920 DeathBlossom
 *   9,866,082,500   8817   1,118,984   2108     4,680,304 ALS_XZ
 *   8,978,274,900   7478   1,200,625   2847     3,153,591 ALS_Wing
 *  14,413,903,800   5220   2,761,284   1912     7,538,652 ALS_Chain
 *   1,222,324,600   3726     328,052     24    50,930,191 SueDeCoq
 * 489,799,433,800   3722 131,595,764    420 1,166,189,128 Superfisch
 *   6,666,204,600   3395   1,963,535    528    12,625,387 UnaryChain
 *   5,441,805,800   3159   1,722,635     23   236,600,252 NishioChain
 *   6,624,083,400   3136   2,112,271   3403     1,946,542 MultipleChain
 *   1,628,317,800   1740     935,814   4155       391,893 DynamicChain
 *      16,656,000      6   2,776,000     24       694,000 DynamicPlus
 * 558,978,623,500
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  612,177,957,900 (10:12)  417,868,913
 *
 * Finned Jellyfish                          	28
 * Finned Franken Jellyfish                  	188
 * Finned Franken Swordfish                  	25
 * Sashimi Swampfish                         	2
 * Finned Swordfish                          	15
 * Sashimi Jellyfish                         	2
 * Finned Mutant Jellyfish                   	61
 * Finned Swampfish                          	6
 *
 * NOTES:
 * 1. 10:12 is 8 minutes too long, but Im not worried about Superfisches
 *    performance. It is what it is.
 * 2. DiufSudoku_V6_30.212.2023-05-23.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-05-26 12:32</b> Permufisherman is try-again Superfischerman.
 * It uses the Permutations class (instead of hobiwans recursion stack), hence
 * hence its rather-odd name.
 * <pre>
 * ran 2023-05-26.00-14-01
 * Superfisch: withoutFins=false withFins=true minSize=2 maxSize=3 maxFins=7
 * maxEndofins=5 maxFishType=2 maxKrakenType=1 maxKrakenSize=3
 *
 *        time(ns)  calls   time/call  elims     time/elim hinter
 *      21,206,100 108314         195  57694           367 NakedSingle
 *      20,013,700  50620         395  16349         1,224 HiddenSingle
 *      83,084,900  34271       2,424  24145         3,441 Locking
 *      95,832,100  22932       4,178   7861        12,190 NakedPair
 *      71,112,100  20865       3,408   8231         8,639 HiddenPair
 *     120,675,500  18792       6,421   1609        75,000 NakedTriple
 *     107,167,200  18394       5,826    995       107,705 HiddenTriple
 *      62,272,500  18196       3,422   1344        46,333 TwoStringKite
 *      50,903,200  16852       3,020    414       122,954 Swampfish
 *      59,236,700  16677       3,551    899        65,891 W_Wing
 *      77,821,700  16048       4,849    532       146,281 XY_Wing
 *      51,944,600  15691       3,310    337       154,138 Skyscraper
 *      99,897,900  15517       6,437    582       171,645 EmptyRectangle
 *      61,544,300  14935       4,120    252       244,223 XYZ_Wing
 *      67,119,600  14698       4,566    245       273,957 Swordfish
 *     153,152,300  14634      10,465    100     1,531,523 NakedQuad
 *      98,104,500  14616       6,712     19     5,163,394 Jellyfish
 *     131,842,400  14612       9,022      1   131,842,400 HiddenQuad
 *     338,357,700  14611      23,157    205     1,650,525 Coloring
 *     934,370,100  14481      64,523    792     1,179,760 XColoring
 *   2,049,590,200  14212     144,215 119286        17,182 GEM
 *   1,272,557,000  12874      98,847    882     1,442,808 UniqueRectangle
 *   3,020,345,200  12248     246,599   2187     1,381,044 BigWings
 *   5,776,205,800  11041     523,159   2572     2,245,803 DeathBlossom
 *  10,239,597,400   8870   1,154,407   2108     4,857,494 ALS_XZ
 *   9,190,256,500   7534   1,219,837   2832     3,245,147 ALS_Wing
 *  15,307,433,100   5297   2,889,830   1908     8,022,763 ALS_Chain
 *   1,317,555,600   3809     345,905     24    54,898,150 SueDeCoq
 * 768,192,417,300   1130 679,816,298    550 1,396,713,486 Permufisherman
 *   6,531,564,500   3335   1,958,490    528    12,370,387 UnaryChain
 *   5,440,659,400   3099   1,755,617    312    17,438,010 NishioChain
 *   6,248,037,500   2805   2,227,464   2826     2,210,912 MultipleChain
 *   1,772,178,200   1689   1,049,247   4041       438,549 DynamicChain
 *      20,450,000      6   3,408,333     24       852,083 DynamicPlus
 * 839,084,506,800
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  914,291,598,900 (15:14)  624,089,828
 *
 * Kraken Type 1: Finned Swordfish           	1
 * Kraken Type 2: Finned Franken Swampfish   	28
 * Kraken Type 1: Finned Swampfish           	28
 * Finned Swordfish                          	2
 * @#Kraken Type 1                           	2 ==== THATS BAD!
 * Kraken Type 2: Finned Franken Swordfish   	15
 * Kraken Type 2: Finned Mutant Swordfish    	4
 * Kraken Type 2: Finned Swampfish           	345
 * Kraken Type 1: Finned Franken Swampfish   	2
 * Kraken Type 1: Finned Mutant Swampfish    	2
 * Kraken Type 2: Franken Swampfish          	7
 * Finned Franken Swordfish                  	2
 * Kraken Type 2: Finned Swordfish           	20
 * Kraken Type 2: Finned Mutant Swampfish    	6
 * Finned Swampfish                          	3
 * Kraken Type 1: Finned Franken Swordfish   	3
 *
 * NOTES:
 * 1. 10:12 is 8 minutes too long, but Im not worried about Superfisches
 *    performance. It is what it is.
 ...
 * <hr>
 * <p>
 * <b>KRC 2023-05-26 19:23</b> Permufisherman is getting there. Unreleasable
 * as of yet. KT1 still has a problem with invalid hints. I dont understand.
 * But this version is worth reverting to if it all goes tits-up, so I build.
 * <pre>
 * ran 2023-05-26.21-20-59
 * FishConfig: withoutFins=false withFins=true minFishSize=2 maxFishSize=3
 * maxFins=7 maxEndofins=5 maxFishType=2 maxKrakenType=1 maxKrakenSize=4
 * maxKrakenSizes=[4, 3, 2]
 *
 *          time(ns)  calls   time/call  elims   time/elim hinter
 *        28,352,500 108446         261  57562         492 NakedSingle
 *        23,992,800  50884         471  16432       1,460 HiddenSingle
 *       103,151,200  34452       2,994  24140       4,273 Locking
 *       116,200,000  23110       5,028   7807      14,884 NakedPair
 *        83,924,600  21060       3,985   8245      10,178 HiddenPair
 *       145,135,400  18984       7,645   1617      89,755 NakedTriple
 *       127,323,900  18579       6,853    987     129,000 HiddenTriple
 *        74,550,300  18388       4,054   1327      56,179 TwoStringKite
 *        59,028,200  17061       3,459    406     145,389 Swampfish
 *        64,931,500  16891       3,844    912      71,196 W_Wing
 *        79,668,200  16261       4,899    550     144,851 XY_Wing
 *        59,468,500  15890       3,742    342     173,884 Skyscraper
 *       116,383,500  15711       7,407    567     205,261 EmptyRectangle
 *        67,588,800  15144       4,463    244     277,003 XYZ_Wing
 *        78,889,700  14916       5,288    226     349,069 Swordfish
 *       189,559,400  14858      12,758     99   1,914,741 NakedQuad
 *       118,611,800  14840       7,992     19   6,242,726 Jellyfish
 *       156,185,900  14836      10,527      1 156,185,900 HiddenQuad
 *       371,088,700  14835      25,014    195   1,903,018 Coloring
 *     1,088,750,200  14710      74,014    851   1,279,377 XColoring
 *     2,384,145,100  14437     165,141 122195      19,510 GEM
 *     1,603,040,900  13071     122,641    881   1,819,569 UniqueRectangle
 *     3,571,845,900  12446     286,987   2149   1,662,096 BigWings
 *     7,055,171,700  11241     627,628   2566   2,749,482 DeathBlossom
 *    12,173,938,300   9073   1,341,776   2108   5,775,113 ALS_XZ
 *    11,248,470,700   7738   1,453,666   2843   3,956,549 ALS_Wing
 *    18,235,456,300   5482   3,326,423   1988   9,172,764 ALS_Chain
 *     1,542,706,400   3929     392,646     20  77,135,320 SueDeCoq
 * 2,008,812,777,800   3926 511,669,072   2767 725,989,439 Permufisherman
 *     3,414,809,500   1716   1,989,982      0           0 UnaryChain
 *     3,669,776,800   1716   2,138,564      4 917,444,200 NishioChain
 *     4,890,404,900   1712   2,856,544    386  12,669,442 MultipleChain
 *     1,798,602,500   1488   1,208,738   3480     516,839 DynamicChain
 *        13,205,200      6   2,200,866     24     550,216 DynamicPlus
 * 2,083,567,137,100
 * pzls         total (ns) (mm:ss)      each (ns)
 * 1465  2,166,635,854,800 (36:06)  1,478,932,324
 *
 * Finned Swampfish                          	12
 * Finned Swordfish                          	9
 * Finned Franken Swordfish                  	9
 * ============ NO TYPE ONES ============
 * Kraken Type 2: Swampfish                  	5
 * Kraken Type 2: Finned Swampfish           	1799
 * Kraken Type 2: Finned Swordfish           	115
 * Kraken Type 2: Franken Swampfish          	11
 * Kraken Type 2: Finned Franken Swampfish   	130
 * Kraken Type 2: Finned Franken Swordfish   	72
 * Kraken Type 2: Finned Mutant Swampfish    	19
 * Kraken Type 2: Finned Mutant Swordfish    	29
 *
 * NOTES:
 * 1. 36:06 is 33 minutes too long, but Im not worried about performance, yet.
 *    What I want at the moment is no invalid KT1 hints, while still producing
 *    some hints, which is not happening. Sigh.
 * 2. DiufSudoku_V6_30.213.2023-05-26.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-05-27 10:37</b>
 * <pre>
 * ran 2023-05-27.10-37-22
 * SuperfischermanByPermutations: withoutFins=false withFins=true minFishSize=2
 * maxFishSize=3 maxFins=7 maxEndofins=5 maxFishType=2 maxKrakenType=1
 * maxKrakenSize=4 maxKrakenSizes=[4, 3, 2]
 *
 *          time(ns)  calls   time/call  elims   time/elim hinter
 *        21,723,000 108446         200  57562         377 NakedSingle
 *        19,377,800  50884         380  16432       1,179 HiddenSingle
 *        80,393,100  34452       2,333  24140       3,330 Locking
 *        89,513,100  23110       3,873   7807      11,465 NakedPair
 *        69,675,400  21060       3,308   8245       8,450 HiddenPair
 *       117,150,500  18984       6,171   1617      72,449 NakedTriple
 *       105,744,100  18579       5,691    987     107,136 HiddenTriple
 *        60,550,800  18388       3,292   1327      45,629 TwoStringKite
 *        49,885,200  17061       2,923    406     122,869 Swampfish
 *        54,742,400  16891       3,240    912      60,024 W_Wing
 *        71,075,400  16261       4,370    550     129,228 XY_Wing
 *        49,139,000  15890       3,092    342     143,681 Skyscraper
 *        98,254,000  15711       6,253    567     173,287 EmptyRectangle
 *        58,845,800  15144       3,885    244     241,171 XYZ_Wing
 *        66,923,900  14916       4,486    226     296,123 Swordfish
 *       151,302,600  14858      10,183     99   1,528,309 NakedQuad
 *        98,505,500  14840       6,637     19   5,184,500 Jellyfish
 *       130,549,000  14836       8,799      1 130,549,000 HiddenQuad
 *       306,423,800  14835      20,655    195   1,571,404 Coloring
 *       896,936,700  14710      60,974    851   1,053,979 XColoring
 *     1,955,598,500  14437     135,457 122195      16,003 GEM
 *     1,268,787,400  13071      97,068    881   1,440,167 UniqueRectangle
 *     2,897,054,000  12446     232,769   2149   1,348,093 BigWings
 *     5,860,892,400  11241     521,385   2566   2,284,057 DeathBlossom
 *    10,334,676,700   9073   1,139,058   2108   4,902,598 ALS_XZ
 *     9,692,888,200   7738   1,252,634   2843   3,409,387 ALS_Wing
 *    15,857,147,900   5482   2,892,584   1988   7,976,432 ALS_Chain
 *     1,327,211,700   3929     337,798     20  66,360,585 SueDeCoq
 * 1,724,691,867,000   3926 439,300,017   2767 623,307,505 SuperfischermanByPermutations
 *     2,969,584,600   1716   1,730,527      0           0 UnaryChain
 *     3,104,889,400   1716   1,809,376      4 776,222,350 NishioChain
 *     4,172,173,800   1712   2,437,017    386  10,808,740 MultipleChain
 *     1,512,003,300   1488   1,016,131   3480     434,483 DynamicChain
 *        16,918,100      6   2,819,683     24     704,920 DynamicPlus
 * 1,788,258,404,100
 * pzls         total (ns) (mm:ss)      each (ns)
 * 1465  1,866,011,511,800 (31:06)  1,273,727,994
 *
 * Finned Swampfish                          	12
 * Finned Swordfish                          	9
 * Kraken Type 2: Swampfish                  	5
 ...
 * Finned Franken Swordfish                  	9
 * Kraken Type 2: Franken Swampfish          	11
 * Kraken Type 2: Finned Franken Swampfish   	149
 * Kraken Type 2: Finned Franken Swordfish   	101
 *
 * NOTES:
 * 1. 31:06 is 28 minutes too long, but performance comes after correctness,
 *    which I think this build has. No test-cases failed and batch produces no
 *    warnings. I removed Kraken Type 1s from searchKrakens. Simples. All KT1s
 *    are found by KT2, so KT1 is/was just a waste of time.
 * 2. DiufSudoku_V6_30.214.2023-05-27.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-05-28</b> SuperfischermanPerm renamed SuperfischerPerm coz still
 * too long to fit in the log. The Kraken Type 1 code has been deleted not just
 * disabled. searchKrakens has been performance tuned.
 * <pre>
 * ran 2023-05-28.16-16-42 SuperfischerPerm: withoutFins=false withFins=true
 * maxFins=5 maxEndofins=3 minFishSize=2 maxFishType=1 maxFishSizes=[4, 3, -1]
 * maxKrakenType=0 maxKrakenSizes=[3, -1, -1]
 *
 *        time(ns)  calls  time/call  elims   time/elim hinter
 *      24,166,300 108297        223  57537         420 NakedSingle
 *      21,394,500  50760        421  16413       1,303 HiddenSingle
 *      85,723,500  34347      2,495  24148       3,549 Locking
 *      94,746,700  23013      4,117   7819      12,117 NakedPair
 *      71,393,000  20965      3,405   8252       8,651 HiddenPair
 *     119,417,600  18890      6,321   1620      73,714 NakedTriple
 *     105,747,200  18487      5,720    996     106,171 HiddenTriple
 *      62,432,200  18292      3,413   1337      46,695 TwoStringKite
 *      51,506,400  16955      3,037    404     127,491 Swampfish
 *      57,877,400  16787      3,447    906      63,882 W_Wing
 *      72,550,200  16168      4,487    544     133,364 XY_Wing
 *      49,697,100  15803      3,144    341     145,739 Skyscraper
 *     106,353,400  15625      6,806    568     187,241 EmptyRectangle
 *      58,904,900  15057      3,912    253     232,825 XYZ_Wing
 *      66,051,000  14820      4,456    221     298,873 Swordfish
 *     150,876,800  14764     10,219     99   1,524,008 NakedQuad
 *      98,016,000  14746      6,646     16   6,126,000 Jellyfish
 *     130,649,200  14743      8,861      1 130,649,200 HiddenQuad
 *     322,122,100  14742     21,850    195   1,651,908 Coloring
 *     911,842,000  14616     62,386    808   1,128,517 XColoring
 *   1,995,423,600  14344    139,112 122362      16,307 GEM
 *   1,277,341,600  12995     98,294    887   1,440,069 UniqueRectangle
 *   3,006,429,000  12364    243,159   2172   1,384,175 BigWings
 *   5,848,029,300  11158    524,110   2561   2,283,494 DeathBlossom
 *  10,360,756,600   8996  1,151,707   2104   4,924,313 ALS_XZ
 *   9,151,536,200   7664  1,194,093   2813   3,253,301 ALS_Wing
 *  15,656,287,200   5428  2,884,356   1963   7,975,693 ALS_Chain
 *   1,320,925,300   3892    339,394     20  66,046,265 SueDeCoq
 * 241,230,500,800   3889 62,028,927   2509  96,146,074 SuperfischerPerm
 *   3,396,058,300   1928  1,761,441      0           0 UnaryChain
 *   3,624,783,400   1928  1,880,074     19 190,778,073 NishioChain
 *   4,711,218,500   1909  2,467,898    503   9,366,239 MultipleChain
 *   1,695,443,100   1604  1,057,009   3848     440,603 DynamicChain
 *      18,530,200      6  3,088,366     24     772,091 DynamicPlus
 * 305,954,730,600
 * pzls        total (ns) (mm:ss)         each (ns)
 * 1465   350,990,912,900 (05:50)       239,584,240
 *
 * Finned Swampfish                          	14
 * Finned Swordfish                          	9
 * Finned Jellyfish                          	1
 * Finned Franken Swordfish                  	24
 * Kraken Swampfish                          	3
 * Kraken Finned Swampfish                   	1800
 * Kraken Finned Swordfish                   	110

 * NOTES:
 * 1. 5:50 is acceptable. It still feels "snappy enough" in the GUI.
 * 2. DiufSudoku_V6_30.215.2023-05-28.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-06-06</b> Superfischerman reverted to KRAKEN_TABLES, for speed.
 * Introduced maxFishSizes=[4,3,2] and maxKrakenSizes=[3,2,-1], for speed.
 * ComplexFisherman and KrakenFisherman use maxFins and maxEndofins CFG.
 * <pre>
 * 2023-06-06.10-34-25 Superfischerman: maxFins=6 maxEndofins=4 maxFishType=2
 * maxFishSizes=[4, 3, 2] maxKrakenType=1 maxKrakenSizes=[3, 2, -1]
 *          time(ns)  calls  time/call  elims     time/elim hinter
 *        39,208,900 107909        363  57247           684 NakedSingle
 *        27,577,100  50662        544  16511         1,670 HiddenSingle
 *       115,443,200  34151      3,380  24106         4,788 Locking
 *       126,040,400  22848      5,516   7815        16,128 NakedPair
 *        96,235,600  20787      4,629   8297        11,598 HiddenPair
 *       172,278,800  18691      9,217   1580       109,037 NakedTriple
 *       150,957,200  18302      8,248    970       155,625 HiddenTriple
 *        81,855,800  18116      4,518   1318        62,106 TwoStringKite
 *        66,135,600  16798      3,937    399       165,753 Swampfish
 *        74,325,800  16633      4,468    872        85,236 W_Wing
 *        98,669,100  16026      6,156    534       184,773 XY_Wing
 *        67,425,800  15671      4,302    339       198,896 Skyscraper
 *       157,096,500  15489     10,142    570       275,607 EmptyRectangle
 *        84,062,600  14919      5,634    237       354,694 XYZ_Wing
 *        87,047,100  14698      5,922    239       364,213 Swordfish
 *       231,942,900  14633     15,850    113     2,052,592 NakedQuad
 *       137,427,200  14612      9,405     16     8,589,200 Jellyfish
 *       182,865,800  14609     12,517      1   182,865,800 HiddenQuad
 *       446,702,700  14608     30,579    202     2,211,399 Coloring
 *     1,312,620,600  14485     90,619    766     1,713,603 XColoring
 *     2,895,502,700  14217    203,664 124599        23,238 GEM
 *     1,932,407,000  12861    150,253    876     2,205,944 UniqueRectangle
 *     4,194,638,700  12231    342,951   2178     1,925,913 BigWings
 *     8,407,431,100  11011    763,548   2539     3,311,315 DeathBlossom
 *    14,261,858,400   8859  1,609,872   2111     6,755,972 ALS_XZ
 *    11,758,343,700   7506  1,566,525   2893     4,064,411 ALS_Wing
 *    20,301,180,000   5212  3,895,084   1900    10,684,831 ALS_Chain
 *     1,702,400,200   3719    457,757     20    85,120,010 SueDeCoq
 *   124,447,142,700   3716 33,489,543   3003    41,440,939 Superfischerman
 *     4,699,137,500   1838  2,556,658      1 4,699,137,500 UnaryChain
 *     4,858,634,400   1837  2,644,874     14   347,045,314 NishioChain
 *     6,586,601,600   1823  3,613,056    435    15,141,612 MultipleChain
 *     2,345,390,800   1568  1,495,784   3535       663,476 DynamicChain
 *        41,987,600      4 10,496,900     22     1,908,527 DynamicPlus
 *   212,188,575,100
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  269,440,088,700 (04:29)  183,918,149
 *
 * Finned Swampfish                           3
 * Sashimi Swampfish                          1
 * Finned Jellyfish                           16
 * Finned Swordfish                           11
 * Finned Franken Swordfish                   26
 * Sashimi Franken Swordfish                  5
 * Finned Mutant Swampfish                    2
 * Sashimi Mutant Swampfish                   1
 * Finned Mutant Swordfish                    3
 * Sashimi Mutant Swordfish                   2
 * Kraken Type 2 Finned Swampfish             116
 * Kraken Type 2 Sashimi Swampfish            493
 * Kraken Type 2 Finned Swordfish             353
 * Kraken Type 2 Sashimi Swordfish            514
 * Kraken Type 2 Finned Franken Swampfish     119
 * Kraken Type 2 Sashimi Franken Swampfish    213
 *
 * NOTES:
 * 1. 4:29 is acceptable. It still feels "fast enough" in the GUI.
 * 2. DiufSudoku_V6_30.216.2023-06-06.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 2023-06-12</b> Bring ComplexFisherman and KrakenFisherman up to speed
 * with learnings from Superfisherman and SuperficherPerm. Both use hobiwans
 * original approach: all sizes and types of fish are sought upto maximums; so
 * LogicalSolver/Tech have all fish flavours back together and TechSelectDialog
 * unselects fish in the same row and col as the selected fish; except Krakens.
 * Krakens are selected separately.
 *
 ...
 * The Superfisch config determine there maxFins and maxEndofins, and there
 * maxSizes/maxKrakenSizes.
 *
 * Also KrakenType1 now disabled in KrakenFisherman (waste of time).
 *
 * AlignedExclusion has also been improved. It was creating millions of garbage
 * CellSets and even more Idxs. AlignedExclusion is now usable time-wise out to
 * A6E, however its still too slow for me to want to actually use it. Also
 * AlignedQuad is now hacked because it was slower than hacked AlignedHex.
 * AlignedExclusion is still not as fast as it should be, but I have not yet
 * been able to turn that feeling into reality. Too stupid.
 *     731,619,000 18524     39,495    35     20,903,400 AlignedPair   H
 *   7,033,715,900 18489    380,427   650     10,821,101 AlignedTriple H
 * 215,266,277,000 17896 12,028,736  1565    137,550,336 AlignedQuad   C
 *  86,639,258,200 16509  5,248,001   538    161,039,513 AlignedPent   H
 * 180,941,839,200 16024 11,291,927   407    444,574,543 AlignedHex    H
 * 264,116,697,500 15671 16,853,850   196  1,347,534,170 AlignedSept   H
 * 296,787,843,300 15491 19,158,727    98  3,028,447,380 AlignedOct    H
 * 238,274,099,800 15408 15,464,310    24  9,928,087,491 AlignedNona   H
 * 147,957,862,800 15385  9,617,020     5 29,591,572,560 AlignedDec    H
 * 1465  1,541,557,696,600 (25:41)  1,052,257,813
 *
 * Also all test-cases use ValidatingHintsAccumulator; to find invalid hints.
 * Also any is(Very)?VeryNaughtyBoys are logged; and sought in two test-cases.
 * <pre>
 * 2023-06-12.13-29-28
 *        time(ns)  calls time/call  elims   time/elim hinter
 *      18,607,500 108073       172  57354         324 NakedSingle
 *      21,286,900  50719       419  16278       1,307 HiddenSingle
 *      86,698,800  34441     2,517  24169       3,587 Locking
 *      96,784,900  23077     4,193   7877      12,287 NakedPair
 *      73,719,900  20999     3,510   8261       8,923 HiddenPair
 *     118,569,600  18922     6,266   1553      76,348 NakedTriple
 *     108,130,700  18538     5,832    987     109,554 HiddenTriple
 *      62,092,000  18345     3,384   1337      46,441 TwoStringKite
 *      53,493,700  17008     3,145    411     130,154 Swampfish
 *      59,290,200  16835     3,521    911      65,082 W_Wing
 *      79,062,100  16198     4,880    527     150,022 XY_Wing
 *      54,660,200  15844     3,449    325     168,185 Skyscraper
 *     104,941,500  15673     6,695    575     182,506 EmptyRectangle
 *      65,040,700  15098     4,307    245     265,472 XYZ_Wing
 *      67,040,800  14869     4,508    229     292,754 Swordfish
 *     151,902,600  14811    10,256    113   1,344,270 NakedQuad
 *     100,912,800  14790     6,823     17   5,936,047 Jellyfish
 *     133,841,000  14786     9,051      1 133,841,000 HiddenQuad
 *     336,357,600  14785    22,749    203   1,656,933 Coloring
 *     918,745,700  14657    62,683    747   1,229,913 XColoring
 *   2,114,927,600  14387   147,002 123009      17,193 GEM
 *   1,314,126,100  13045   100,737    890   1,476,546 UniqueRectangle
 *   3,051,814,800  12408   245,955   2188   1,394,796 BigWings
 *   6,000,219,800  11185   536,452   2566   2,338,355 DeathBlossom
 *  10,669,708,500   9012 1,183,944   2119   5,035,256 ALS_XZ
 *   9,590,574,600   7650 1,253,669   2929   3,274,351 ALS_Wing
 *  15,719,663,000   5316 2,957,047   1889   8,321,685 ALS_Chain
 *   1,348,026,400   3844   350,683     24  56,167,766 SueDeCoq
 *   5,528,871,000   3840 1,439,810     55 100,524,927 KrakenSwampfish
 *  31,025,975,800   3800 8,164,730    320  96,956,174 MutantSwordfish
 *  33,412,625,100   3544 9,427,941     74 451,521,960 FrankenJellyfish
 *   7,271,654,800   3481 2,088,955    512  14,202,450 UnaryChain
 *   6,213,302,400   3263 1,904,168     23 270,143,582 NishioChain
 *   7,466,003,700   3240 2,304,322   3611   2,067,572 MultipleChain
 *   1,891,466,800   1753 1,078,988   4017     470,865 DynamicChain
 *      23,246,500      4 5,811,625     22   1,056,659 DynamicPlus
 * 145,353,386,100
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  187,575,158,600 (03:07)  128,037,650
 *
 * Finned Swampfish                           16
 * Finned Swordfish                           63
 * Finned Jellyfish                           3
 * Finned Franken Swordfish                   56
 * Finned Franken Jellyfish                   60
 * Finned Mutant Swordfish                    121
 * Kraken Type 2 Finned Swampfish             4
 * Kraken Type 2 Sashimi Swampfish            35
 * Kraken Type 2 Sashimi Franken Swampfish    1
 *
 * NOTES:
 * 1. 3:07 is acceptable with Big Fish, but maxFins=6 and maxEndofins=4, so do
 *    not get exited, it is not fixed, merely less broken.
 * 2. DiufSudoku_V6_30.217.2023-06-12.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.218 2023-06-16</b> Old Aligned*Exclusion_(1C|2H) got chopped.
 * The newer maintainable AlignedExclusion is now the align package. The old
 * wing.BigWing class got chopped in favour of als.BigWings. SuperfischerPerm
 * is history: slower. Now I swap difficulty from double to int.
 * <pre>
 *       time(ns)  calls  time/call  elims   time/elim hinter
 *     23,572,900 108668        216  57346         411 NakedSingle
 *     20,124,000  51322        392  16385       1,228 HiddenSingle
 *     83,458,000  34937      2,388  24139       3,457 Locking
 *    104,736,900  23573      4,443   7816      13,400 NakedPair
 *     74,590,900  21513      3,467   8225       9,068 HiddenPair
 *    139,419,600  19430      7,175   1587      87,851 NakedTriple
 *    108,869,000  19033      5,720    983     110,751 HiddenTriple
 *     62,460,100  18843      3,314   1346      46,404 TwoStringKite
 *     53,810,500  17497      3,075    437     123,136 Swampfish
 *     60,728,700  17309      3,508    890      68,234 W_Wing
 *     79,992,000  16693      4,791    532     150,360 XY_Wing
 *     52,347,500  16339      3,203    318     164,614 Skyscraper
 *    112,533,600  16176      6,956    576     195,370 EmptyRectangle
 *     63,189,700  15600      4,050    255     247,802 XYZ_Wing
 *     66,075,300  15360      4,301    219     301,713 Swordfish
 *    178,683,000  15300     11,678    120   1,489,025 NakedQuad
 *     93,175,500  15278      6,098     17   5,480,911 Jellyfish
 *    135,240,000  15274      8,854      1 135,240,000 HiddenQuad
 *    342,481,300  15273     22,423    196   1,747,353 Coloring
 *    942,385,400  15150     62,203    778   1,211,292 XColoring
 *  2,067,750,800  14879    138,971 124506      16,607 GEM
 *  1,350,912,600  13494    100,112    894   1,511,087 UniqueRectangle
 *  3,127,772,900  12857    243,273   2108   1,483,763 BigWings
 *  6,236,737,600  11673    534,287   2592   2,406,148 DeathBlossom
 * 10,764,641,500   9483  1,135,151   2169   4,962,951 ALS_XZ
 * 10,256,229,500   8115  1,263,860   2877   3,564,904 ALS_Wing
 * 17,086,059,300   5830  2,930,713   1969   8,677,531 ALS_Chain
 *  1,337,799,600   4273    313,082     24  55,741,650 SueDeCoq
 *  6,096,647,000   4269  1,428,120     51 119,542,098 KrakenSwampfish
 * 34,689,312,000   4234  8,193,035    328 105,760,097 MutantSwordfish
 * 37,278,893,100   3967  9,397,250     76 490,511,751 FrankenJellyfish
 *  7,726,079,300   3901  1,980,538    293  26,368,871 UnaryChain
 *  6,630,692,900   3677  1,803,288     23 288,290,995 NishioChain
 *  8,663,002,800   3654  2,370,827   3218   2,692,045 MultipleChain
 *  6,346,306,500   2137  2,969,726   2927   2,168,194 DynamicChain
 *    125,754,200     11 11,432,200     11  11,432,200 DynamicPlus
 * 1465  204,278,512,400 (03:24)  139,439,257
 *
 * NOTES:
 * 1. 3:24 is acceptable with Big Fish and maxFins=7 and maxEndofins=5.
 * 2. DiufSudoku_V6_30.218.2023-06-16.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.219 2023-06-17</b> Swap difficulty from double to int.
 * <pre>
 *       time(ns)  calls  time/call  elims     time/elim hinter
 *     25,572,100 108972        234  57653           443 NakedSingle
 *     19,850,900  51319        386  16445         1,207 HiddenSingle
 *     81,399,800  34874      2,334  24069         3,381 Locking
 *    100,384,000  23561      4,260   7796        12,876 NakedPair
 *     71,934,200  21498      3,346   8244         8,725 HiddenPair
 *    139,056,600  19410      7,164   1572        88,458 NakedTriple
 *    110,452,500  19013      5,809    972       113,634 HiddenTriple
 *     63,104,800  18828      3,351   1332        47,375 TwoStringKite
 *     52,235,000  17496      2,985    434       120,357 Swampfish
 *     60,208,000  17312      3,477    904        66,601 W_Wing
 *     79,541,800  16688      4,766    532       149,514 XY_Wing
 *     50,838,400  16334      3,112    335       151,756 Skyscraper
 *    105,253,500  16161      6,512    568       185,305 EmptyRectangle
 *     60,631,500  15593      3,888    248       244,481 XYZ_Wing
 *     65,414,100  15362      4,258    224       292,027 Swordfish
 *    175,610,600  15302     11,476    119     1,475,719 NakedQuad
 *     92,418,500  15280      6,048     17     5,436,382 Jellyfish
 *    139,652,800  15276      9,141      1   139,652,800 HiddenQuad
 *    350,743,200  15275     22,961    194     1,807,954 Coloring
 *    938,897,600  15159     61,936    878     1,069,359 XColoring
 *  2,084,212,000  14880    140,068 122698        16,986 GEM
 *  1,359,458,900  13498    100,715    901     1,508,833 UniqueRectangle
 *  3,096,134,700  12856    240,831   2105     1,470,847 BigWings
 *  6,150,123,900  11665    527,228   2590     2,374,565 DeathBlossom
 * 10,763,572,300   9480  1,135,397   2141     5,027,357 ALS_XZ
 * 10,105,245,700   8130  1,242,957   2898     3,486,972 ALS_Wing
 * 16,835,942,600   5823  2,891,283   1962     8,581,010 ALS_Chain
 *  1,386,799,500   4284    323,716     20    69,339,975 SueDeCoq
 * 78,926,393,000   4281 18,436,438   2681    29,439,161 KrakenSwampfish
 * 18,680,029,800   2197  8,502,516     18 1,037,779,433 MutantSwordfish
 * 21,354,894,000   2180  9,795,822     21 1,016,899,714 FrankenJellyfish
 *  3,844,117,800   2160  1,779,684      0             0 UnaryChain
 *  3,983,044,400   2160  1,844,002      2 1,991,522,200 NishioChain
 *  5,575,100,700   2158  2,583,457    427    13,056,441 MultipleChain
 *  5,536,135,600   1920  2,883,403   2548     2,172,737 DynamicChain
 *    120,819,700     11 10,983,609     11    10,983,609 DynamicPlus
 * 1465  235,352,248,600 (03:55)  160,649,999
 *
 * Finned Swordfish                          3
 * Finned Jellyfish                          1
 * Finned Franken Swordfish                  3
 * Finned Franken Jellyfish                  19
 * Finned Mutant Swordfish                   11
 * Kraken Type 2 Finned Swampfish            188
 * Kraken Type 2 Sashimi Swampfish           796
 * Kraken Type 2 Finned Franken Swampfish    137
 * Kraken Type 2 Sashimi Franken Swampfish   375
 * Kraken Type 2 Finned Mutant Swampfish     167
 * Kraken Type 2 Sashimi Mutant Swampfish    421
 *
 * NOTES:
 * 1. 3:55 is up about 30 seconds, and I do not know why. I am going to blame
 *    a hot box, and we shall see next time. I note its finding more Krakens.
 * 2. DiufSudoku_V6_30.219.2023-06-17.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.220 2023-06-18</b> Clean-up bugs; esp ComplexFisherman usedBases.
 * This one is a release candidate.
 * <pre>
 *       time(ns) calls  time/call  elims     time/elim hinter
 *     23,444,700 08734        215  57400           408 NakedSingle
 *     20,109,500 51334        391  16385         1,227 HiddenSingle
 *     86,543,300 34949      2,476  24145         3,584 Locking
 *    101,358,200 23581      4,298   7816        12,968 NakedPair
 *     71,678,900 21521      3,330   8224         8,715 HiddenPair
 *    138,850,500 19439      7,142   1588        87,437 NakedTriple
 *    114,305,300 19042      6,002    983       116,282 HiddenTriple
 *     63,898,200 18852      3,389   1346        47,472 TwoStringKite
 *     56,823,700 17506      3,245    437       130,031 Swampfish
 *     59,141,500 17318      3,415    893        66,227 W_Wing
 *     81,145,100 16700      4,858    536       151,390 XY_Wing
 *     51,641,300 16344      3,159    319       161,884 Skyscraper
 *    106,278,800 16180      6,568    575       184,832 EmptyRectangle
 *     58,609,600 15605      3,755    254       230,746 XYZ_Wing
 *     74,555,300 15366      4,851    219       340,435 Swordfish
 *    177,338,200 15306     11,586    120     1,477,818 NakedQuad
 *    111,898,100 15284      7,321     17     6,582,241 Jellyfish
 *    138,121,200 15280      9,039      1   138,121,200 HiddenQuad
 *    339,826,800 15279     22,241    196     1,733,810 Coloring
 *    932,928,200 15156     61,555    766     1,217,921 XColoring
 *  2,036,666,800 14888    136,799 123964        16,429 GEM
 *  1,325,636,800 13507     98,144    897     1,477,855 UniqueRectangle
 *  3,044,146,200 12867    236,585   2108     1,444,092 BigWings
 *  6,095,763,500 11683    521,763   2598     2,346,329 DeathBlossom
 * 10,887,562,700  9489  1,147,387   2164     5,031,221 ALS_XZ
 * 10,294,284,100  8126  1,266,832   2877     3,578,131 ALS_Wing
 * 16,952,496,600  5841  2,902,327   1974     8,587,890 ALS_Chain
 *  1,395,487,700  4279    326,124     24    58,145,320 SueDeCoq
 * 42,188,809,200  4275  9,868,727    346   121,932,974 MutantSwordfish
 * 62,961,394,900  3987 15,791,671     62 1,015,506,369 FrankenJellyfish
 *  7,472,631,100  3932  1,900,465    336    22,239,973 UnaryChain
 *  6,430,768,700  3674  1,750,345     22   292,307,668 NishioChain
 *  7,803,882,900  3652  2,136,879   3216     2,426,580 MultipleChain
 *  6,006,619,300  2137  2,810,771   2927     2,052,141 DynamicChain
 *    119,607,900    11 10,873,445     11    10,873,445 DynamicPlus
 * 1465  229,282,875,600 (03:49)  156,507,082
 * BASIC
 * Finned Swampfish                          14
 * Finned Swordfish                          63
 * Sashimi Swordfish                         6
 * Finned Jellyfish                          2
 * FRANKEN
 * Finned Franken Swordfish                  46
 * Sashimi Franken Swordfish                 9
 * Finned Franken Jellyfish                  43
 * Sashimi Franken Jellyfish                 10
 * MUTANT
 * Finned Mutant Swampfish                   1
 * Sashimi Mutant Swampfish                  1
 * Finned Mutant Swordfish                   63
 * Sashimi Mutant Swordfish                  85
 *
 * NOTES:
 * 1. 3:49 is OK with big fish. Anything under ~5 mins feels OK in the GUI.
 * 2. DiufSudoku_V6_30.220.2023-06-18.LOGS.7z
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.221 2023-06-29</b> Added a LogFollow feature to GUI. Clean-up:
 * eradicated unused methods. Added Als.vBudsX exploded idx and hinters. I must
 * have changed other stuff, but I will be buggered if I can remember what.
 * <pre>
 *       time(ns)  calls time/call  elims   time/elim hinter
 *     23,310,900 108347       215  57154         407 NakedSingle
 *     20,258,400  51193       395  16342       1,239 HiddenSingle
 *     84,613,900  34851     2,427  24120       3,508 Locking
 *     96,665,300  23509     4,111   7830      12,345 NakedPair
 *     70,734,900  21448     3,297   8219       8,606 HiddenPair
 *    138,390,200  19373     7,143   1570      88,146 NakedTriple
 *    108,610,100  18980     5,722    978     111,053 HiddenTriple
 *     60,352,700  18791     3,211   1331      45,343 TwoStringKite
 *     55,256,100  17460     3,164    422     130,938 Swampfish
 *     61,465,300  17279     3,557    884      69,530 W_Wing
 *     83,340,400  16669     4,999    548     152,081 XY_Wing
 *     50,470,600  16309     3,094    327     154,344 Skyscraper
 *    108,002,300  16139     6,692    566     190,816 EmptyRectangle
 *     61,421,600  15573     3,944    257     238,994 XYZ_Wing
 *     74,887,300  15331     4,884    231     324,187 Swordfish
 *    174,290,500  15266    11,416    110   1,584,459 NakedQuad
 *    109,301,000  15245     7,169     17   6,429,470 Jellyfish
 *    135,543,200  15241     8,893      1 135,543,200 HiddenQuad
 *    338,629,700  15240    22,219    188   1,801,221 Coloring
 *    969,461,900  15117    64,130    779   1,244,495 XColoring
 *  2,091,966,800  14846   140,911 126575      16,527 GEM
 *  1,325,455,400  13429    98,700    901   1,471,093 UniqueRectangle
 *  3,811,284,400  12785   298,105   2153   1,770,220 BigWings
 *  6,441,760,700  11573   556,619   2567   2,509,450 DeathBlossom
 * 11,564,712,400   9399 1,230,419   2131   5,426,894 ALS_XZ
 * 11,417,975,500   8046 1,419,087   2896   3,942,671 ALS_Wing
 * 17,622,023,000   5744 3,067,900   1989   8,859,740 ALS_Chain
 *  1,323,047,400   4180   316,518     24  55,126,975 SueDeCoq
 * 16,730,286,200   4176 4,006,294    325  51,477,803 FrankenSwordfish
 *  7,564,320,200   3923 1,928,197    332  22,784,096 UnaryChain
 *  6,617,846,100   3660 1,808,154     95  69,661,537 NishioChain
 *  7,591,496,700   3565 2,129,452   3367   2,254,676 MultipleChain
 *  2,016,876,500   2043   987,213   2941     685,779 DynamicChain
 *     25,559,500     11 2,323,590     11   2,323,590 DynamicPlus
 * 98,969,617,100
 * 1465  139,746,244,500 (02:19)  95,389,927
 *
 * NOTES:
 * 1. 2:19 is OK. Anything under ~5 mins feels OK in the GUI.
 * 2. DiufSudoku_V6_30.221.2023-06-29.LOGS.7z is releasable AFAIK.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.222 2023-07-03</b> I am building now before trying changing the
 * core of Idx from 3 ints to an array of 3 ints, so that Idx is semi-exploded.
 * This release has only minor changes. Exploded Als.vBudsX being the biggest.
 * <pre>
 * 2023-07-03.05-48-49
 * WantedEnabled: NakedSingle, HiddenSingle, Locking, NakedPair, HiddenPair, NakedTriple, HiddenTriple, TwoStringKite, Swampfish, W_Wing, XY_Wing, Skyscraper, EmptyRectangle, XYZ_Wing, Swordfish, NakedQuad, Jellyfish, HiddenQuad, Coloring, XColoring, GEM, UniqueRectangle, BigWings, DeathBlossom, ALS_XZ, ALS_Wing, ALS_Chain, SueDeCoq, FrankenSwordfish, UnaryChain, NishioChain, MultipleChain, DynamicChain, DynamicPlus, NestedUnary, NestedMultiple, NestedDynamic
 * Unwanted: Medusa3D, FinnedSwampfish, FrankenSwampfish, MutantSwampfish, KrakenSwampfish, FinnedSwordfish, MutantSwordfish, KrakenSwordfish, FinnedJellyfish, FrankenJellyfish, MutantJellyfish, KrakenJellyfish, AlignedPair, AlignedTriple, AlignedQuad, AlignedPent, AlignedHex, AlignedSept, AlignedOct, AlignedNona, AlignedDec, NestedPlus
 * Errors: none
 *        time(ns)  calls time/call  elims   time/elim hinter
 *      26,425,400 108347       243  57154         462 NakedSingle
 *      21,857,800  51193       426  16342       1,337 HiddenSingle
 *      91,526,600  34851     2,626  24120       3,794 Locking
 *     115,455,000  23509     4,911   7830      14,745 NakedPair
 *      76,566,200  21448     3,569   8219       9,315 HiddenPair
 *     147,987,900  19373     7,638   1570      94,259 NakedTriple
 *     121,577,800  18980     6,405    978     124,312 HiddenTriple
 *      68,423,000  18791     3,641   1331      51,407 TwoStringKite
 *      59,948,900  17460     3,433    422     142,059 Swampfish
 *      68,002,000  17279     3,935    884      76,925 W_Wing
 *      90,171,400  16669     5,409    548     164,546 XY_Wing
 *      55,977,000  16309     3,432    327     171,183 Skyscraper
 *     118,065,200  16139     7,315    566     208,595 EmptyRectangle
 *      63,966,200  15573     4,107    257     248,895 XYZ_Wing
 *      79,632,000  15331     5,194    231     344,727 Swordfish
 *     188,332,500  15266    12,336    110   1,712,113 NakedQuad
 *     117,584,900  15245     7,713     17   6,916,758 Jellyfish
 *     148,217,900  15241     9,724      1 148,217,900 HiddenQuad
 *     380,270,500  15240    24,952    188   2,022,715 Coloring
 *   1,019,558,400  15117    67,444    779   1,308,804 XColoring
 *   2,205,967,000  14846   148,589 126575      17,428 GEM
 *   1,410,187,800  13429   105,010    901   1,565,136 UniqueRectangle
 *   4,003,790,000  12785   313,163   2153   1,859,633 BigWings
 *   6,830,308,300  11573   590,193   2567   2,660,813 DeathBlossom
 *  12,401,165,600   9399 1,319,413   2131   5,819,411 ALS_XZ
 *  12,360,223,800   8046 1,536,194   2896   4,268,033 ALS_Wing
 *  18,906,281,400   5744 3,291,483   1989   9,505,420 ALS_Chain
 *   1,509,062,300   4180   361,019     24  62,877,595 SueDeCoq
 *  17,781,339,700   4176 4,257,983    325  54,711,814 FrankenSwordfish
 *   8,025,872,600   3923 2,045,850    332  24,174,315 UnaryChain
 *   8,290,511,700   3660 2,265,167     95  87,268,544 NishioChain
 *   8,235,859,000   3565 2,310,198   3367   2,446,052 MultipleChain
 *   2,309,663,600   2043 1,130,525   2941     785,332 DynamicChain
 *      26,557,000     11 2,414,272     11   2,414,272 DynamicPlus
 * 107,356,336,400
 * pzls       total (ns) (mm:ss)    each (ns)
 * 1465  151,324,566,600 (02:31)  103,293,219
 *
 * NOTES:
 * 1. 2:31 is OK. Anything under ~5 mins feels OK in the GUI.
 * 2. DiufSudoku_V6_30.222.2023-07-03.LOGS.7z is releasable AFAIK.
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.223 2023-07-12</b> AlsFinderPlain fastardised. Drop AlsChainFast
 * because AlsChainGui was faster, and more capable. DeathBlossom fastardised.
 * ChainerMulti fastardised. Note if you remove "if XChain and YChain" out of
 * doChains its faster, but not accurate. Do so if you care not for accuracy.
 * <pre>
 *       time(ns)  calls time/call  elims   time/elim hinter
 *     23,298,600 108319       215  57176         407 NakedSingle
 *     41,618,000  51143       813  16324       2,549 HiddenSingle
 *     79,112,100  34819     2,272  24118       3,280 Locking
 *     98,246,600  23473     4,185   7840      12,531 NakedPair
 *     68,560,800  21409     3,202   8206       8,354 HiddenPair
 *    135,992,500  19342     7,030   1579      86,125 NakedTriple
 *    112,240,600  18947     5,923    979     114,648 HiddenTriple
 *     57,543,100  18758     3,067   1326      43,396 TwoStringKite
 *     55,288,200  17432     3,171    426     129,784 Swampfish
 *     60,779,500  17249     3,523    890      68,291 W_Wing
 *     81,400,300  16634     4,893    547     148,812 XY_Wing
 *     49,202,300  16275     3,023    324     151,858 Skyscraper
 *    105,354,600  16106     6,541    566     186,138 EmptyRectangle
 *     57,068,500  15540     3,672    258     221,195 XYZ_Wing
 *     73,330,500  15297     4,793    230     318,828 Swordfish
 *    174,351,800  15233    11,445    110   1,585,016 NakedQuad
 *    107,163,600  15212     7,044     17   6,303,741 Jellyfish
 *    138,181,500  15208     9,086      1 138,181,500 HiddenQuad
 *    322,376,000  15207    21,199    183   1,761,617 Coloring
 *    954,801,500  15082    63,307    766   1,246,477 XColoring
 *  1,996,643,900  14815   134,771 126508      15,782 GEM
 *  1,307,602,100  13414    97,480    902   1,449,669 URT
 *  3,812,478,100  12770   298,549   2150   1,773,245 BigWings
 *  3,960,262,000  11557   342,672   2501   1,583,471 DeathBlossom
 * 10,839,001,000   9433 1,149,051   2158   5,022,706 ALS_XZ
 *  9,901,520,900   8068 1,227,258   2895   3,420,214 ALS_Wing
 * 15,754,101,400   5769 2,730,820   2029   7,764,465 ALS_Chain
 *  1,380,164,000   4205   328,219     24  57,506,833 SueDeCoq
 * 18,557,539,700   4201 4,417,410    326  56,924,968 FrankenSwordfish
 *  7,357,094,600   3947 1,863,971    341  21,575,057 UnaryChain
 *  7,327,341,400   3675 1,993,834     95  77,129,909 NishioChain
 *  6,245,510,200   3580 1,744,555   3417   1,827,775 MultipleChain
 *  1,868,112,800   2044   913,949   2942     634,980 DynamicChain
 *     14,824,200     11 1,347,654     11   1,347,654 DynamicPlus
 * 93,118,106,900
 * pzls       total (ns) (mm:ss)   each (ns)
 * 1465  142,035,800,700 (02:22)  96,952,764
 *
 * NOTES:
 * 1. 2:22 is faster coz BigWings is faster coz AlsFinderPlain is faster; and
 *    coz ALS_Chain is faster coz I chucked AlsChainFast coz it was SLOWER; and
 *    DeathBlossom is faster coz I nailed it. ChainerMulti is faster because it
 *    now has its own COMPLETE doChains method, eliminating stackwork.
 * 2. DiufSudoku_V6_30.223.2023-07-12.LOGS.7z is a release
 * </pre>
 * <hr>
 * <p>
 * <b>KRC 6.30.224 2023-07-14</b> Fastardised the chainers. doChains is now all
 * one method with no onToOffs/offToOns methods for speed. Split ChainerMulti
 * to ChainerMultiple, ChainerNishio, and ChainerDynamic rather than waste time
 * on all those isMultiple, isDynamic, etc decisions. I am a bit proud of this.
 * Its completely unmaintainable, but fast. Exactly how you want it. Chainers
 * are now in lock-down.
 * <pre>
 *       time(ns)  calls time/call  elims  time/elim hinter
 *     23,313,000 108026       215  57068        408 NakedSingle
 *     38,934,700  50958       764  16484      2,361 HiddenSingle
 *     78,744,800  34474     2,284  24135      3,262 Locking
 *     99,277,000  23121     4,293   7820     12,695 NakedPair
 *     67,476,600  21076     3,201   8224      8,204 HiddenPair
 *    132,689,100  18996     6,985   1572     84,407 NakedTriple
 *    111,035,400  18608     5,967    989    112,270 HiddenTriple
 *     59,116,300  18410     3,211   1321     44,751 TwoStringKite
 *     53,116,200  17089     3,108    411    129,236 Swampfish
 *     56,780,700  16912     3,357    932     60,923 W_Wing
 *     79,504,800  16274     4,885    520    152,893 XY_Wing
 *     46,977,100  15930     2,948    334    140,650 Skyscraper
 *    106,528,400  15753     6,762    577    184,624 EmptyRectangle
 *     54,747,900  15176     3,607    260    210,568 XYZ_Wing
 *     65,817,000  14932     4,407    241    273,099 Swordfish
 *    170,300,700  14864    11,457     95  1,792,638 NakedQuad
 *     90,397,200  14846     6,088     16  5,649,825 Jellyfish
 *    135,209,600  14843     9,109      4 33,802,400 HiddenQuad
 *    311,264,500  14841    20,973    178  1,748,676 Coloring
 *    917,255,200  14722    62,305    784  1,169,968 XColoring
 *  1,937,909,100  14465   133,972 126804     15,282 GEM
 *  1,327,143,300  13138   101,015    890  1,491,172 URT
 *  3,803,646,900  12501   304,267   2149  1,769,961 BigWings
 *  4,018,449,800  11310   355,300   2492  1,612,540 DeathBlossom
 * 10,575,050,300   9201 1,149,337   2170  4,873,295 ALS_XZ
 *  9,721,580,300   7827 1,242,057   2825  3,441,267 ALS_Wing
 * 15,061,856,300   5578 2,700,225   1994  7,553,588 ALS_Chain
 *  1,377,885,300   4009   343,698     15 91,859,020 SueDeCoq
 * 17,970,117,500   4006 4,485,800    321 55,981,674 FrankenSwordfish
 *  5,967,163,900   3758 1,587,856    340 17,550,482 UnaryChain
 *  6,565,202,300   3483 1,884,927     92 71,360,894 NishioChain
 *  2,350,745,100   3391   693,230   5315    442,285 DynamicChain
 *     18,137,900     11 1,648,900     11  1,648,900 DynamicPlus
 * 83,393,374,200
 * pzls       total (ns) (mm:ss)   each (ns)
 * 1465  131,124,735,600 (02:11)  89,504,938
 *
 * NOTES:
 * 1. 2:11 is 11 seconds faster coz the chainers are MUCH faster.
 * 2. DiufSudoku_V6_30.224.2023-07-14.7z is releasable
 * </pre>
 */
final class BuildTimings_01 {

	private BuildTimings_01() { } // Never used
