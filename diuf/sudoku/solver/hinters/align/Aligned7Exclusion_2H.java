 ...
 * ============================================================================
 *
 * The hacked version of Aligned 7 (Sept) Exclusion technique.
...
//		final Cell[] scells = new Cell[degree];
		...
		Idx idx0; // bitset of positions of c0's excluder cells in Grid.cells
		...
		c0=c1=c2=c3=c4=c5=c6 = null; // just for debug
		...
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		//             more variables than Jessie the bloody elephant
		//                     (but my trunks a lot shorter)
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		final int n0 = numCandidates - 6;
		final int n1 = numCandidates - 5;
		final int n2 = numCandidates - 4;
		final int n3 = numCandidates - 3;
		final int n4 = numCandidates - 2;
		final int n5 = numCandidates - 1;
		for ( i0=0; i0<n0; ++i0 ) {
...
//KRC#2020-06-30 09:50:00
					...
					for ( i3=i2+1; i3<n3; ++i3 ) {
						if ( excluders[candidates[i3].i].idx2(idx02, idx03) )
							continue;
...
//KRC#2020-06-30 09:50:00
						...
						for ( i4=i3+1; i4<n4; ++i4 ) {
							if ( excluders[candidates[i4].i].idx2(idx03, idx04) )
								continue;
...
//KRC#2020-06-30 09:50:00
							...
							for ( i5=i4+1; i5<n5; ++i5 ) {
								if ( excluders[candidates[i5].i].idx2(idx04, idx05) )
									continue;
...
//KRC#2020-06-30 09:50:00
								...
								for ( i6=i5+1; i6<numCandidates; ++i6 ) {
									if ( excluders[candidates[i6].i].idx2(idx05, idx06) )
										continue;
...
// cmnExcls/cells
// Tim.small/Tim.small  :     30,537,679,700	   4471	     6,830,167	      5	 6,107,535,940	Aligned Sept
// Bubble-Sort/no-sort  :     27,861,304,700	   4471	     6,231,559	      5	 5,572,260,940	Aligned Sept
// redo                 :     27,473,132,300	   4471	     6,144,739	      5	 5,494,626,460	Aligned Sept
// bSort method/no-sort :     28,061,886,900	   4471	     6,276,422	      5	 5,612,377,380	Aligned Sept
//									//MyTimSort.small(cmnExcls, numCmnExcls, Grid.BY_MAYBES_SIZE);
									bubbleSort(cmnExcls, numCmnExcls);

									// get common excluder bits, minus any supersets,
									// eg: {125,12}=>{12} coz 125 covers 12, so 125 does nada.
...
++counts[numCmnExclBits];
									...
									case 1:
										...
										//  w/o HitSet: 1 = 4,849,264 of 21,396,286 = 22.66%
										// with HitSet: 1 = 1 of 164 = 0.61%
														...
														SV4_LOOP: for ( int sv4 : SVS[c4b3] ) {
															...
															SV5_LOOP: for ( int sv5 : SVS[c5b4] ) {
																...
																SV6_LOOP: for ( int sv6 : SVS[c6b5] ) {
																	...
																	if ( (avb6|=sv6) != c6b )
																		continue;
																	// We allow only the FIRST combo at each level; then
																	// move-on to evaluating the next value at this level,
																	// which has not yet been evaluated; instead of wasting
																	// time evaluating multiple combos-to-my-right which
																	// can not allow any additional maybes. This is faster,
																	// reducing: O(n*n*n*n*n*n*n) to O(n*n*n'ish).
																	switch ( anyLevel ) {
									...
									default:
										...
										//  w/o HitSet: 3 = 680,461 of 21,396,286 = 3.18%
										// with HitSet: 3 = 3 of 164 = 1.83%
										//nb: do60 is allways true. Why do we never find a hint using
										// a common excluder with more than 6 potential values?
										// Why not 7? or 8? How to figure it out? I know knuffink!
										DOG_1c: for ( int sv0 : SVS[c0b] ) {
											...
											c2b0 = ns20 ? c2b : c2b & ~sv0;
											any1 = false;
											// nb: c1b0 is 0 so rarely it's not worth testing for
											SV1_LOOP: for ( int sv1 : SVS[ns10 ? c1b : c1b & ~sv0] ) {
												sv01 = sv0 | sv1;
												c6b1 = ns61 ? c6b0 : c6b0 & ~sv1;
												c5b1 = ns51 ? c5b0 : c5b0 & ~sv1;
												...
												c3b1 = ns31 ? c3b0 : c3b0 & ~sv1;
												any2 = false;
												// nb: c2b1 is 0 so rarely it's not worth testing for
												SV2_LOOP: for ( int sv2 : SVS[ns21 ? c2b0 : c2b0 & ~sv1] ) {
													sv02 = sv01 | sv2;
													if ( do30 && covers(cmnExclBits, numCmnExclBits, sv02)
													  // nb: HOWEVER c4b3 is 0 often enough to make it worth testing for
													  || (c3b2 = ns32 ? c3b1 : c3b1 & ~sv2) == 0 )
														continue;
													c6b2 = ns62 ? c6b1 : c6b1 & ~sv2;
													c5b2 = ns52 ? c5b1 : c5b1 & ~sv2;
													c4b2 = ns42 ? c4b1 : c4b1 & ~sv2;
													any3 = false;
													// nb: c3b2 is 0 so rarely it's not worth testing for
													SV3_LOOP: for ( int sv3 : SVS[c3b2] ) {
														sv03 = sv02 | sv3;
...
++hintCounts[numCmnExclBits];
									...
									//KRC#2020-06-30 10:20:00 (scells or cells)
									Pots redPots = createRedPotentials(cells
											, avb0, avb1, avb2, avb3, avb4, avb5, avb6);
									if ( redPots.isEmpty() )
										continue; // Should never happen. Never say never.// Should never happen. Never say never.//Netbeans is a piece of s__t!
