 ...
 * from the other (non-box) end of the boxs other col/row. This sounds wrong!
 * <p>
 * The logic is that the box must contain a v, which is also in either the row
 * or the col, so if it's in the row/col which intersects the biplace region
 * then the other place in that region must be v, so the victim (intersection
 * of "the other place"s cross-region and the box's other col/row) cannot be v;
 ...
 * to view an EmptyRectangle hint; a picture is worth a thousand words, and the
 * hint explanation is clearer than this over-abstracted pile of puss.
 * <p>
 ...
 * expressions to handle the role reversal of rows/cols and x/y, but couldn't
 * workout how to get the victim cell. Problem is swap row and col roles. This
 * technique is less code, but I guarantee it'll be slower because it'll call
 * 6 million more methods, each with 5 lambdas that may be invoked; ergo slower
 * than a bloody turkey in a greyhound race. What turkey?
 *
 * KRC 2022-01-19 Attempt to make it a bit faster.
 * PRE: 104,704,800  14783  7,082  567  184,664  EmptyRectangle 1:14 ????
 * removed the useless bitCount call from c2y/x tautology in both big if's.
...
//	private static final long[] COUNTS_A = new long[6], COUNTS_B = new long[6];
	 ...
	 * the box.ridx[v].bits (a bitset of box.cells indexes which may be v)
	 * which must <b>NOT</b> maybe v in order for this box to match this
	 * Empty&nbsp;Rectangle pattern. Note well that NOT!
	 * <p>
	 * Foreach er pattern 0..8: {@code erBox.ridx[v].bits & EMPTY_CELLS[er]==0}
	 ...
	 * ridx[v].bits; so I translated them over. That is, I translated hobiwans
	 ...
	 * </pre>
	 * <p>
	 * Sorry but these bitsets are upside down, and left is right.
	 * <p>
	 ...
	 * <p>
	 * A computer is like wiping your ass with wet-and-dry: I'll swap you four
	 * 320's for a 1200. This is a 320 solution. Grin and bear it. Either that
	 * or reindex your head from EOF, which is NOT how s__t works. Davo's just
	 * buffed his knob off again. Obviously, he's off to the ER. Pretty piss
	 * poor joke that, but on with the show. You'll want two mirrors and some
	 * alternate physics. They're upside down! And right is left! And these are
	 * cells which NOT maybe v, in the generic erBox, not a specific box. Jesus
	 * my ____ing brain hurts, but it's done now. Sigh.
	 */
	private static final int[] EMPTY_CELLS = {
	...
	/**
	 * Masks used to determine if all v's in a box are in the same row.
	 * <p>
	...
	/**
	 * Masks used to determine if all v's in a box are in the same col.
	 * <p>
	 * This is a right-to-left representation of a left-to-right reality,
	...
	/** The erRow index by [bi][er] (ie [boxIndex][erPatternIndex]). */
	private static final int[][] ER_ROWS = new int[REGION_SIZE][REGION_SIZE];

	/** The erCol index by [bi][er] (ie [boxIndex][erPatternIndex]). */
	...
	public boolean findHints(final Grid grid, final IAccumulator accu) {
		...
		int bi // boxIndex
		  ...
		  // nb: erRow and erCol are the lines of 0s in EMPTY_CELLS bitsets;
		  // ie erBox has 2..5 vs and they're all in erRow and/or erCol.
		  , erR // erRow index
		  , erC // erCol index
		...
		// foreach erBox
		for ( bi=0; bi<REGION_SIZE; ++bi ) { // boxIndex
			// foreach value
			for ( erBoxRidx=(erBox=gBoxs[bi]).ridx,v=1; v<VALUE_CEILING; ++v ) { // value
				sv = VSHFT[v]; // shiftedValue
				// An ER starts from an erBox with 2..5 vs
				if ( (card=erBoxRidx[v].size)>1 && card<6 ) {
					erBoxVs = erBoxRidx[v].bits; // bitset of vs in erBox.cells
					// foreach ER pattern
					PATTERN: for ( er=0; er<REGION_SIZE; ++er ) { // ER index
						// if erBoxVs are all in erRow or erCol (no others).
						if ( (erBoxVs & EMPTY_CELLS[er]) == 0
						  // and erBoxVs are NOT all in the erRow
						  && (erBoxVs & ~ROWS[(erR=ER_ROWS[bi][er])%R]) != 0
						  // and erBoxVs are NOT all in the erCol
						  && (erBoxVs & ~COLS[(erC=ER_COLS[bi][er])%R]) != 0
						  // then erBox has vs in both the erRow and the erCol,
						  // and no others, so the v in the erBox must be in
						  // either the erRow or the erCol (or possibly both,
						  // which does not effect the ER logic).
						) {
							erRow = gRows[erR];
							erCol = gCols[erC];
							...
							// just a little bit faster (thanks mostly to the
							// generic EMPTY_CELLS, not to no method, I think).
							//
							...
							// most abstract (least code) possible solution.
							// Sometimes one gets ones knuckles dirty. What I
							// really want is the brains to write code to write
							// optimal compiled machine code, so I fiddle with
							// the fur around the edge instead. Pointless.
							//
...
//&& ++COUNTS_A[3] > 0L // 112,837
								  // and tautology to set c2y
								  && (c2y=crvs.bits & ~ISHFT[c1.y]) > -1
...
//++COUNTS_A[5]; // 293
									...
									if ( accu.add(new EmptyRectangleHint(this, v
										...
										return result;
									break PATTERN; // skip remaining patterns
								}
							}
...
//&& ++COUNTS_B[3] > 0L // 114,496
								  // and tautology to set c2x
								  && (c2x=crvs.bits & ~ISHFT[c1.x]) > -1
